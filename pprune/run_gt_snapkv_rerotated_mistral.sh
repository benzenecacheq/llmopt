...
echo "=== GT snapkv_rerotated Mistral (65/50/35%) started at $(date) ==="
python gt_eval_compression.py --model mistralai/Mistral-7B-v0.3 --tasks "$TASKS" \
    --methods snapkv_rerotated \
    --output lb_results_base/gt_snapkv_rerotated_mistral \
    --n 100
python gt_eval_compression.py --model mistralai/Mistral-7B-v0.3 --tasks "$TASKS" \
    --methods snapkv_rerotated_f50 \
    --output lb_results_base/gt_snapkv_rerotated_mistral_f50 \
    --n 100
python gt_eval_compression.py --model mistralai/Mistral-7B-v0.3 --tasks "$TASKS" \
    --methods snapkv_rerotated_f35 \
    --output lb_results_base/gt_snapkv_rerotated_mistral_f35 \
    --n 100
echo "=== done at $(date) ==="
