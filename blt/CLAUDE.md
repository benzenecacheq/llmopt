...
**Second sine-schedule seed launched (2026-07-27).** `run_gpt2_ema_blend75_sine_scratch_seed19.pt` on `venus` (idle after the seed42 run finished) — seed 19 (matches the existing fixed-blend75 family's seed sequence for eventual direct comparison), otherwise identical config to the seed42 sine run. Confirmed healthy at step 0 (`effective_blend=0.0000`, as expected). In progress.
...
- **`bender`** (local): GPT-2 **medium** (355M, 24 layers/16 heads/1024 dim) baseline from-scratch OWT, seed 42, running — `run_gpt2_medium_baseline_seed42.pt`, target **1.5M steps**, on `--dataset openwebtext_large` (single-epoch 75-file corpus, no repetition) after two rounds of OOM debugging in the dataset loader (see "GPT-2 medium" below for the full story — model-size code changes, batch-size fix, thermal-throttling analysis, the resume data-replay bug, the corpus-repetition fix, and the two OOM bugs in `openwebtext_large`'s loader). **Power outage 2026-07-20 (attempted fan swap that didn't fit) killed this at step 153,500; resumed cleanly via `--resume`, progressing past step 174,600 as of last check — see "Power-loss outage and resume" above.**
- **`titan`**: GPT-2 baseline seed19 **DONE** (2026-07-19), final val_ppl 56.72 — third baseline seed to match BLT's 42/19/7. Its watcher fired as designed and launched the queued BLT seed19-at-500K run (`run_blt_scratch_seed19_500k.pt`), which then also **completed (2026-07-23)**, final WikiText val_ppl 82.37, 355,988s. Benchmarked 2026-07-25 (both OWT held-out and lm-eval-harness run concurrently on `titan` with no crash, despite the documented resource-contention risk on `bender` from doing this — worth remembering titan tolerates it fine): OWT held-out ppl **32.87** (loss 3.4927), LAMBADA acc 0.208, LAMBADA ppl 343.4, HellaSwag acc_norm 0.271, PIQA acc_norm 0.559, Winogrande acc 0.500 (`lm_eval_blt_scratch_seed19_500k.json`). Direct comparison against GPT-2 baseline seed19 (same seed, same 500K steps): BLT is 16.4% worse on OWT ppl (32.87 vs 28.24) but **exactly ties** on LAMBADA acc (0.208 vs 0.208) and edges ahead on HellaSwag (0.271 vs 0.266); GPT-2 wins PIQA (0.572 vs 0.559) and Winogrande (0.520 vs 0.500) — the same qualitative trade-off pattern seen in every other BLT-vs-baseline seed comparison. This is the weakest of the three iso-step (500K) BLT seeds on OWT ppl (32.87 vs seed42_500k's 31.69 and seed7's 30.81) but not an outlier — LAMBADA acc (0.208) lands mid-pack (vs 0.188/0.212), consistent with ordinary seed variance, not a bug. **This completes the "at least two/three genuine iso-step seeds per architecture" goal for BLT** (now 42_500k/19_500k/7), matching GPT-2 baseline (42/19/7) and GQA (42/seed3-group-42/7). `titan` is now free — the "nothing else on titan" constraint from this run is over.
