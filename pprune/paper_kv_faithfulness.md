...
|---|---|---|---|---|---|---|
| 65% | 0.175 | 0.141 | 1.157 | **0.043** | 1.394 | **0.043** |
| 50% | 0.217 | 0.199 | 0.981 | **0.077** | 1.451 | **0.077** |
| 35% | 0.281 | 0.261 | 0.836 | **0.124** | 0.837 | **0.124** |

SnapKV+rot achieves dramatically lower KL than any other method at every rate, except Pyr+rot, which is identical. Re-rotating retained keys to compact positions after eviction reduces mean KL from 1.157 to 0.043 at 65% retention — a 27× improvement. The finding that Pyr+rot = SnapKV+rot at all rates (to three decimal places, across all 16 tasks) confirms that position correction, not token selection strategy, drives this improvement.
...
| Retention | Naive | Streaming | SnapKV   | SnapKV+rot | Pyr      | Pyr+rot |
|-----------|-------|-----------|----------|------------|----------|---------|
| 65%       | 68.6  | 61.2      | 78.5     | 71.1       | **79.1** | 71.1    |
| 50%       | 52.1  | 56.4      | **73.2** | 67.8       | 64.2     | 67.8    |
...
SnapKV+rot (71.1%, 67.8%, 63.0%) falls below the post-hoc eviction leaders but substantially above Naive. At 50% it exceeds PyramidKV (67.8 vs 64.2). The striking result is that Pyr+rot = SnapKV+rot on every task at every rate: once positional displacement is corrected, the selection strategy (attention-score vs. layer-adaptive budget) has no measurable effect on output faithfulness.

Streaming achieves 61.2% / 56.4% / 51.8% (65/50/35%), consistently above Naive but well below the post-hoc eviction leaders.

**Mistral-7B-v0.3** (65% and 35% retention).

| Retention | Naive | Streaming | SnapKV | SnapKV+rot | Pyr |
|---|---|---|---|---|---|
| 65% | 61.1 | 61.0 | 83.7 | 78.2 | **83.9** |
| 35% | 53.5 | 53.9 | 74.0 | 70.1 | **74.8** |

Mistral shows the same qualitative pattern as Llama. PyramidKV leads F_out at both rates (83.9%, 74.8%); SnapKV is close (83.7%, 74.0%); SnapKV+rot (78.2%, 70.1%) is below unrotated SnapKV/Pyr but above Naive and Streaming, mirroring the Llama finding that rerotation improves KL at the cost of some F_out.
...
## 8. Why Post-Hoc KV Eviction Leads F_out but Trails KL
...
The practical implication is direct. Post-hoc eviction cannot reduce TTFT, so SnapKV+rot's quality advantage is concentrated precisely where latency savings matter least: short exact-lookup tasks where generation time is negligible and prefill dominates total cost. On long-form tasks, SnapKV and Pyr fall to or below Naive's long-form F_out (Naive: 57.6%, Pyr: 55.5%, SnapKV: 54.7%) while Naive delivers a 20% decode speedup with no prefill overhead or KL corruption. There is no deployment regime in which any post-hoc eviction method is simultaneously much faster than uncompressed and more faithful than naive truncation.
...
|---|---|---|---|---|---|---|
| NarrativeQA† | 0.022 | 0.045 | 0.787 | **0.010** | 1.337 | **0.010** |
| Qasper† | 0.281 | 0.237 | 0.881 | **0.092** | 1.229 | **0.092** |
| MultifieldQA | 0.262 | 0.302 | 1.316 | **0.037** | 1.535 | **0.037** |
| HotpotQA† | 0.203 | 0.069 | 1.935 | **0.006** | 2.181 | **0.006** |
| 2WikiMQA† | 0.221 | 0.103 | 1.714 | **0.008** | 2.422 | **0.008** |
| MuSiQue† | 0.197 | 0.098 | 1.747 | **0.007** | 2.523 | **0.007** |
| GovReport | 0.280 | 0.385 | 0.532 | **0.128** | 0.334 | **0.128** |
| QMSum† | 0.081 | 0.093 | 0.393 | **0.055** | 0.438 | **0.055** |
| MultiNews | 0.585 | 0.643 | 1.050 | **0.322** | 0.721 | **0.322** |
| TREC | 0.202 | 0.084 | 1.454 | **0.011** | 1.526 | **0.011** |
...
| PassageRetrieval | 0.188 | 0.032 | 1.624 | **0.002** | 1.416 | **0.002** |
| LCC | 0.076 | 0.085 | 0.881 | **0.007** | 1.142 | **0.007** |
| RepoBench-P | 0.048 | 0.050 | 0.702 | **0.004** | 1.052 | **0.004** |
| **Average** | 0.175 | 0.141 | 1.157 | **0.043** | 1.394 | **0.043** |

**Table A1b. 50% retention.**

| Task | Naive | Streaming | SnapKV | SnapKV+rot | Pyr | Pyr+rot |
|---|---|---|---|---|---|---|
| NarrativeQA† | 0.026 | 0.068 | 0.806 | **0.019** | 1.380 | **0.019** |
| Qasper† | 0.457 | 0.447 | 0.810 | **0.165** | 1.394 | **0.165** |
| MultifieldQA | 0.359 | 0.417 | 1.024 | **0.065** | 1.619 | **0.065** |
| HotpotQA† | 0.212 | 0.097 | 1.519 | **0.012** | 2.245 | **0.012** |
| 2WikiMQA† | 0.278 | 0.198 | 1.286 | **0.021** | 2.282 | **0.021** |
| MuSiQue† | 0.188 | 0.123 | 1.457 | **0.013** | 2.507 | **0.013** |
| GovReport | 0.327 | 0.466 | 0.488 | **0.223** | 0.504 | **0.223** |
| QMSum† | 0.088 | 0.125 | 0.369 | **0.086** | 0.484 | **0.086** |
| MultiNews | 0.759 | 0.784 | 1.002 | **0.551** | 1.099 | **0.551** |
| TREC | 0.260 | 0.183 | 1.114 | **0.016** | 1.711 | **0.016** |
| TriviaQA | 0.095 | 0.015 | 0.963 | **0.001** | 1.622 | **0.001** |
| SAMSum | 0.012 | 0.020 | 0.619 | **0.008** | 1.199 | **0.008** |
| PassageCount† | 0.055 | 0.003 | 1.468 | **0.000** | 1.538 | **0.000** |
| PassageRetrieval | 0.188 | 0.043 | 1.504 | **0.004** | 1.378 | **0.004** |
| LCC | 0.105 | 0.114 | 0.677 | **0.029** | 1.238 | **0.029** |
| RepoBench-P | 0.065 | 0.088 | 0.580 | **0.011** | 1.016 | **0.011** |
| **Average** | 0.217 | 0.199 | 0.981 | **0.077** | 1.451 | **0.077** |
...
| 2WikiMQA† | 0.351 | 0.238 | 1.040 | **0.048** | 1.052 | **0.048** |
...
| MultiNews | 0.947 | 0.961 | 1.063 | **0.822** | 1.064 | **0.822** |
...
| Task             | Naive  | Streaming | SnapKV | SnapKV+rot | Pyr   |
|------------------|--------|-----------|--------|------------|-------|
| NarrativeQA†     | **0.011** | 0.170  | 0.203  | 0.022      | 0.203 |
| Qasper†          | 0.165  | 0.165     | 0.262  | **0.086**  | 0.262 |
| MultifieldQA     | 0.213  | 0.286     | 0.519  | **0.077**  | 0.513 |
| HotpotQA†        | 0.218  | 0.310     | 0.632  | **0.028**  | 0.633 |
| 2WikiMQA†        | 0.362  | 0.365     | 0.690  | **0.051**  | 0.684 |
| MuSiQue†         | 0.186  | 0.290     | 0.679  | **0.041**  | 0.687 |
| GovReport        | 0.263  | 0.399     | 0.457  | **0.248**  | 0.457 |
| QMSum†           | 0.084  | 0.114     | 0.175  | **0.058**  | 0.175 |
| MultiNews        | 0.900  | 0.937     | 1.040  | **0.750**  | 1.039 |
| TREC             | 70.0 | 60.0     | 63.0      | **69.0** | 68.0       | **69.0** | 68.0     |
| TriviaQA         | 17.4 | 17.3     | **18.3**  | 17.4     | 17.6       | 17.4     | 17.6     |
...
| Task             | Full | Naive    | Streaming | SnapKV   | Pyr      |
| ---------------- | ---- | -------- | --------- | -------- | -------- |
| NarrativeQA†     | 5.1  | 2.7      | **5.4**   | 5.1      | 4.1      |
| Qasper†          | 5.3  | 4.3      | 5.0       | **5.1**  | 4.8      |
| MultifieldQA     | 25.3 | 22.1     | 23.2      | **24.9** | 24.8     |
| HotpotQA†        | 10.5 | 10.6     | 9.8       | **10.7** | 10.6     |
| 2WikiMQA†        | 11.5 | 9.8      | 11.3      | **11.7** | **11.7** |
| MuSiQue†         | 5.1  | 3.9      | 4.4       | **5.1**  | **5.1**  |
| GovReport        | 20.0 | 19.7     | 19.4      | 19.8     | **20.0** |
| QMSum†           | 8.0  | **8.4**  | 8.3       | 7.9      | 7.9      |
| MultiNews        | 18.1 | 17.2     | 17.1      | 17.2     | **17.3** |
| TREC             | 72.0 | 67.0     | 70.0      | 70.0     | **71.0** |
| TriviaQA         | 23.1 | 24.2     | **26.5**  | 23.4     | 23.3     |
| SAMSum           | 16.9 | 17.8     | **18.3**  | 17.9     | 18.1     |
| PassageCount†    | 1.0  | **3.0**  | 1.0       | 1.0      | 1.0      |
| PassageRetrieval | 39.0 | 26.0     | 30.0      | **39.0** | **39.0** |
| LCC              | 62.9 | 60.2     | 62.1      | **63.2** | **63.2** |
| RepoBench-P      | 53.9 | **54.4** | 54.1      | 54.1     | 54.0     |
| **Average**      | 23.6 | 22.0     | **22.9**  | **23.5** | **23.5** |

**Table C2b. 35% retention.**

| Task             | Full | Naive    | Streaming | SnapKV   | Pyr      |
| ---------------- | ---- | -------- | --------- | -------- | -------- |
| NarrativeQA†     | 5.1  | 3.9      | 4.7       | **5.1**  | **5.1**  |
| Qasper†          | 5.3  | 4.0      | 4.2       | **4.9**  | 4.8      |
| MultifieldQA     | 25.3 | 18.2     | 18.6      | **25.1** | **25.1** |
| HotpotQA†        | 10.5 | 11.0     | **11.3**  | 10.4     | 10.4     |
| 2WikiMQA†        | 11.5 | 11.6     | **12.4**  | 11.2     | 11.2     |
| MuSiQue†         | 5.1  | 4.6      | 3.5       | **5.2**  | **5.2**  |
| GovReport        | 20.0 | **18.6** | 18.0      | **18.6** | **18.6** |
| QMSum†           | 8.0  | 7.8      | **8.2**   | 7.8      | 7.8      |
| MultiNews        | 18.1 | **16.5** | 15.2      | 15.7     | 15.7     |
| TREC             | 72.0 | 60.0     | 61.0      | **66.0** | **66.0** |
| TriviaQA         | 23.1 | 25.3     | **26.9**  | 23.7     | 23.7     |
| SAMSum           | 16.9 | 17.2     | **18.3**  | 17.8     | 17.8     |
| PassageCount†    | 1.0  | **3.0**  | 1.0       | 1.0      | 1.0      |
| PassageRetrieval | 39.0 | 15.0     | 22.0      | **39.0** | **39.0** |
| LCC              | 62.9 | 57.4     | **62.6**  | 62.3     | 62.3     |
| RepoBench-P      | 53.9 | 49.5     | 52.1      | **53.7** | **53.7** |
| **Average**      | 23.6 | 20.2     | 21.2      | **23.0** | **23.0** |
