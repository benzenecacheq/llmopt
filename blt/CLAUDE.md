...
- `bender`: real medium `num_uv_groups=8` run, step 620,550/1,500,000 (41.4%).
- `titan`: fixed EMA blend=1.0 seed19, step 436,670/500,000 (87.3%).
...
**Cumulative mode is different in exactly the way that matters here**: since it's an exact running mean (not a fixed-decay window), its own variance genuinely shrinks as sample count grows (~1/n), independent of LR. So a sine anneal applied to cumulative mode isn't correcting for the same (now-doubted) mechanism it was designed for with fixed-decay EMA — its effect, if any, is unclear and worth testing as its own question rather than assumed to carry over.

**Launched as an exploratory test on `titan`** (small model, cheap test vehicle): sine+cumulative had never been combined before (sine was previously only ever paired with fixed-decay EMA). Smoke-tested first (100 steps): `effective_blend` followed the exact sine formula (0.7071 at step 50/100, 0.9877 at step 90/100 — exact match to `sin(0.5·π·s/100)` at target blend 1.0), loss declining normally (10.98→9.15, no NaN), and the buffer showed real differentiation (29,083/50,257 tokens touched, `ema_loss` std 0.399) — confirms the two features compose correctly. Cleaned up smoke artifacts, launched the real run: `run_gpt2_cumulative_sine_scratch_seed42.pt` — `train.py --baseline --from-scratch --ema-loss-weighting --loss-weighting-mode cumulative --ema-blend 1.0 --ema-blend-schedule sine --dataset openwebtext --seed 42 --max-steps 500000 --eval-every 1000 --lambada-eval-every 5000 --owt-eval-every 5000`. Confirmed genuinely running via `ps aux`. Compare against the existing pure-cumulative blend=1.0 3-seed result (OWT ppl 30.09 avg, LAMBADA acc 0.265 avg) once finished — this is exploratory, not yet a multi-seed-confirmed test.

### Bender thermal check-in (2026-09-10) — no active throttling found, but worth periodic re-checks given the machine's history

User flagged a concern that `bender` (running the medium `num_uv_groups=8` job, ~343h/14.3 days in as of this check) looked like it was overheating again. Checked directly rather than assuming either way, per the project's own repeated lesson about not trusting a single point-in-time reading:

- **Live telemetry, 5 samples over ~50s**: 82°C, clock 1335–1492 MHz (87–97% of the 1530 MHz rated max) — nowhere near the historically-documented severe throttling signature (620–800 MHz, 40–52% of max). `nvidia-smi -q` confirms `SW Thermal Slowdown: Not Active` and `HW Slowdown: Not Active` at the time of checking. Temp is comfortably below both the 87°C slowdown threshold and 90°C shutdown threshold.
- **Full training-log step-timing history** (all ~343 hours, 15 sampled windows spread across the entire run): remarkably stable, 1455–1720 ms/step throughout, with the two isolated higher readings (1720 at ~74h, 1610 at ~271h) not part of any sustained trend — the most recent window (1495 ms/step at 342.5h) is on the faster end, not slower. No progressive-throttling signature of the kind seen in past incidents (which showed compounding multi-hour slowdowns).

**Read: not currently throttling, and no evidence of a developing trend in this run's own history — but this doesn't rule out something the live numbers don't capture** (e.g. physical heat/fan noise the user may be observing directly, or a transient spike between samples). Given bender's real history of intermittent throttling episodes, worth taking the concern seriously rather than dismissing it outright — flagged back to the user rather than closed out unilaterally.

### Benchmark-reweighting experiment: does standard lm-eval scoring dilute the EMA/cumulative signal? (2026-09-11/12)

**Motivating question, from the user.** EMA/cumulative training deliberately does not score all tokens equally — so why should the benchmarks used to *evaluate* it score every token in a candidate completion equally via a plain summed log-likelihood? If common/low-information tokens dilute the benchmark signal the same way they dilute the raw training loss, reweighting benchmark scoring the same way might reveal a bigger EMA advantage than the standard metrics show.

**Token list, chosen deliberately narrow and exclude-on-any-doubt.** Considered POS-tag-based generation first; a naive single-word (no sentence context) tagger run over the GPT-2 vocab produced unusable noise (real content words like "mammoth"/"tooth" misclassified, and real risk words — nor/either/neither/unless/until/without/though/since/while/because/for/but/or — showed up in the naive candidate lists). Settled on a small, hand-reasoned closed-class list instead: **a, an, the, of, and** — deliberately excluding all quantifiers/demonstratives (no/any/some/all/each/every/this/that — real logical/deictic weight), all other conjunctions (or/but/nor/yet/so — each changes logical structure), and all other prepositions (in/on/at/under/before/after/with/without/for — real spatial/temporal/causal content). Restricted to **leading-space GPT-2 BPE token variants only** (10 ids: 257/262/281/286/290/317/383/843/1052/3226) — the bare no-space variants were tested and rejected after finding a real collision (`"theater"` tokenizes as `['the','ater']`, reusing the bare "the" token as a content fragment; leading-space tokens don't have this problem since a BPE merge including a literal space only occurs at genuine word boundaries — verified against a broader word list with no collisions found).

**Implementation** (`blt_lm_eval.py`, commit `d5b79fd`): `--lowimpact-weight` (default 1.0, exact prior behavior — verified bit-for-bit identical to pre-patch code via `git stash` comparison) scales the low-impact tokens' contribution to the accumulated NLL in `BLTModel.loglikelihood()`. **Complete removal (weight=0) was considered and rejected** — checked directly against real BLiMP data first: `existential_there_quantifiers_1` and `superlative_quantifiers_2` both have minimal pairs where the *entire* distinguishing signal is one of these five words (e.g. "There was **a** documentary..." vs "...**each** documentary...") — zeroing it out would turn those specific comparisons into an exact tie, destroying real signal for reasons unrelated to the hypothesis being tested. Used a swept weight range (1.0/0.5/0.25/0.1/0.0) instead of one fixed value, per user's request, specifically to see whether reweighting shows a real trend or not.

**Result: mixed, no clean confirmation of the hypothesis.** Swept baseline vs. cumulative-blend=1.0 (seed42 each) across all 8 benchmarks:

| Benchmark | Structural fit for the hypothesis | Result |
|---|---|---|
| BLiMP | Summed multi-token comparison, but minimal-pair design means most of the sentence is shared between candidates | Gap stays flat (−0.021 at every weight) — both raw scores drift down together, comparison unaffected |
| HellaSwag | Full-sentence candidate comparison | No real gap exists between these two checkpoints at *any* weight (±0.003, within stderr) — nothing to reveal |
| **PIQA** | Full-sentence candidate comparison | **Real, direction-consistent narrowing**: gap shrinks from −0.017 (w=1.0) to −0.005 (w=0.25/0.1), though not perfectly monotonic (partially reopens at w=0.0) |
| Winogrande | Full-sentence candidate comparison | Flat small gap regardless of weight — no real baseline-vs-EMA difference to begin with |
| **ARC-Easy** | Full-sentence candidate comparison | **Moves the wrong way** — gap widens slightly (−0.007 → −0.011) as weight decreases |
| BoolQ | `doc_to_choice: ["no","yes"]` — single-token answers | **Mechanically inert**: identical score at every weight (neither "yes" nor "no" is in the 5-word set) |
| OpenBookQA | Full-sentence, but only 500 examples | Bounces with no discernible pattern — 0.2pt-per-example granularity, too noisy to read |
| LAMBADA | `output_type: loglikelihood`, `acc` computed from `is_greedy` (single top-1 comparison, no weighted sum) | **Mechanically inert on `acc`** (bit-for-bit identical at every weight, confirmed empirically) — only `perplexity` moves, and this project has never treated LAMBADA perplexity as the metric that matters |

**Verdict**: one real supporting result (PIQA), one real opposing result (ARC-Easy), two structurally inert for reasons unrelated to the hypothesis (BoolQ, LAMBADA), and the rest flat/too noisy. Doesn't rise to "standard benchmarks structurally hide the EMA advantage" — reads more like occasional benchmark-specific sensitivity to which five words happen to get touched. Result files: `lm_eval_lowimpact_sweep_*.json` (40 files: 5 weights × 2 checkpoints × 4 benchmark groupings).

### ARC-Easy per-item decomposition: the apparent loss is statistically indistinguishable from noise (2026-09-12)

Following on from the sweep above, the user asked to look at specific ARC-Easy examples where cumulative loses to baseline, since ARC-Easy's aggregate gap (baseline 0.377 vs cumulative 0.369) looked like one of the two "clean losses" alongside BLiMP. Verified first that ARC-Easy scores literal fixed candidate text (`doc_to_choice: "{{choices.text}}"`), not a paraphrase-matching problem as initially hypothesized.

**Full per-item comparison, all 2,376 test examples, baseline vs. cumulative-α1.0** (script: ad hoc, not committed — see `arc_easy_full_results.json`/`arc_easy_full_results_medium.json`):

| Outcome | Count (small scale) | Count (medium scale, baseline vs. EMA sine-α0.75) |
|---|---|---|
...
| Other right, baseline wrong | 164 (6.9%) | **161 (6.8%)** |

**McNemar's test on the disagreement counts: χ²=1.04, p=0.31 (small scale); χ²=0.000, p=1.0000 (medium scale — an exact 161-161 tie).** Statistically indistinguishable from a coin flip at both scales. Decomposing the score deltas further: in the two disagreement categories, whichever candidate (gold or the wrong pick) happens to catch a large idiosyncratic score swing wins that item — the two categories are near-exact mirror images of each other in magnitude (small scale: gold −2.91/wrong-pick −0.62 in one direction, gold −0.40/wrong-pick −2.60 in the other). In the two "both agree" categories (76-78% of the data, not selected on anything unusual), gold's score change is not significantly different from the general background drift (t-tests: p=0.41/0.12 small scale, p=0.35/0.87 medium scale) — no systematic effect on the correct answer at all outside the disagreement pool. **Conclusion: ARC-Easy's apparent ~1pt loss is noise, not a real cost of the loss-reweighting family** — a genuinely different situation from BLiMP (below), and a useful data point against reading every small aggregate gap in this project's benchmark tables as necessarily real.

### BLiMP per-item decomposition: a real, concentrated cost — and a real scoring bug caught along the way (2026-09-12)

**Bug found and fixed before trusting any of this.** Initial ad hoc per-item BLiMP scoring didn't reproduce the officially-reported per-subtask accuracy (0.317 vs. the harness's real 0.384 for `left_branch_island_echo_question`) — traced to a missing `target_delimiter` (the harness prepends a single leading space between an empty context and each BLiMP candidate sentence; the ad hoc script passed the raw sentence with no leading space, causing GPT-2's byte-level BPE to tokenize each sentence's first word differently than the real benchmark does). Fixed and verified exact reproduction (0.384) before proceeding. **This exact same fix (`' ' + text` on both `sentence_good`/`sentence_bad`) is required for any future ad hoc BLiMP scoring script — the pattern won't be obviously wrong, since it still returns a plausible-looking accuracy number, just a corrupted one.**

**Pooled result, all 67,000 BLiMP items, baseline vs. cumulative-α1.0**: baseline 0.7806, cumulative 0.7644. McNemar χ²=136.9, p≈0 — a real, highly significant effect (unlike ARC-Easy above). 41/67 subtasks individually significant, lopsided 28-worse vs. 13-better (not a balanced bidirectional pattern — the earlier buggy run had shown a roughly-balanced 21 significant, which is exactly the kind of result the bug produced and should have been more suspicious about at the time).

**The losses cluster in two coherent long-range-dependency categories**: Binding/Anaphora (Principle A: `principle_A_reconstruction` −12.3pt, `principle_A_c_command` −11.2pt, `principle_A_domain_1` −6.6pt) and Movement/Islands (wh-movement across clause boundaries: `wh_questions_object_gap` −12.0pt, `wh_questions_subject_gap_long_distance` −9.5pt, `sentential_subject_island` −9.8pt). **The gains cluster in NPI licensing** (a more local phenomenon, typically resolved within a clause): `only_npi_scope` +11.1pt, `npi_present_2` +6.0pt, `npi_present_1` +4.2pt. Interpretation offered (falsifiable, not yet independently tested): the per-token gradient reallocation that produces the LAMBADA benefit specifically costs long-range structural tracking, not grammaticality in general. Written up in `paper_ema.md` Section 4.6 (committed).

**Medium-scale replication check (baseline vs. EMA sine-α0.75, not a matched mechanism/blend to the small-scale cumulative-α1.0 result — cumulative hasn't finished at medium scale yet)**: pooled effect replicates (χ²=48.9, p≈0, 15-worse/6-better). Category-level picture is genuinely mixed — Binding/Anaphora and Quantifiers replicate cleanly in direction and rough magnitude (Binding: −3.04% small / −1.68% medium, same 3-worse/0-better split at both scales); Movement/Islands keeps direction but shrinks to ~1/5 its small-scale size; **NPI Licensing's clean small-scale gain (+3.57%, 5-better/1-worse) does not replicate — it flips to net-negative at medium scale (−1.94%, 2-and-2)**, with individual subtasks reversing sign entirely (`only_npi_licensor_present`: +3.3% small → −12.1% medium). One seed at each scale, and a mechanism/blend mismatch — read as "the overall concentrated-cost finding replicates, Binding/Anaphora is the most robust piece of it, the NPI-licensing consolation story should not be assumed to generalize."
...
**Migrated to `titan` (2026-09-19)** since `io` got tied up with an unrelated `pprune/` job (`kl_faith_eval_ystar.py`) with no known ETA, and the checkpoint had otherwise been sitting idle for 3 days. Checkpoint re-verified safe immediately before moving it (step 414,500, all finite, `token_count` sum 1,696,142,184 across 50,139/50,257 tokens — unchanged since the pause, confirming nothing touched it in the interim). Relayed through `bender`'s local scratchpad (io has no direct SSH path to titan), md5sum-verified byte-identical at both hops (`867f5caae4b15a9beacfadef7ecbba5b`). `titan`'s repo was pulled clean first (one byte-identical untracked-file collision on the sine+cumulative benchmark results, resolved via `md5sum` the usual way) and the `openwebtext_large` 75-file cache (33.9GB) copied over from `bender` (md5sum `dd84d7564...`, matches every other machine's copy) since titan had only ever run small-model (`openwebtext`, 21-file) experiments before.

Resumed via `--resume ... --batch-size 2 --grad-accum-steps 2 --resume-batch-size 4` — titan's P100 is also a 16GB card, so it needs the same batch=2/grad-accum=2 config bender used for its own medium runs; `--resume-batch-size 4` tells the `ResumableSampler` the checkpoint's dataloader position was recorded under the original batch=4 (from `io`), so it picks up the correct sample offset rather than reinterpreting `micro_step` under the new batch size. Confirmed exactly correct: `Resumed at step 414500`, `effective_blend=1.0000` (correct for this fixed blend=1.0 run), loss reads sane (4.27, 4.35 nats) for this point in training. `nvidia-smi` itself is broken on titan right now (NVML driver/library version mismatch, the same class of issue seen before on bender/io) — a monitoring-tool problem only, not a training problem: `torch.cuda.is_available()` succeeded and the loss is genuinely progressing, so left alone rather than rebooting titan (which would kill the job just launched).
