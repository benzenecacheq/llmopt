...
### Current status snapshot (2026-07-28) — read this first for hand-off
- **`bender`** (local): GPT-2 **medium** (355M) baseline from-scratch OWT, seed 42, still running — `run_gpt2_medium_baseline_seed42.pt`, target **1.5M steps**, past step 690,000 (46% through) as of last check. Survived the 2026-07-20 power outage via `--resume` (see "Power-loss outage and resume" below for that whole story, plus the earlier OOM/thermal/data-replay saga in "GPT-2 medium" below). This is the one stable, uneventful long-runner among the four machines right now.
- **`titan`**: free since 2026-07-23 (GPT-2 baseline seed19 and BLT seed19-at-500K both completed and benchmarked — see "BLT seed19-at-500K bookkeeping" below for the full results and the direct BLT-vs-baseline comparison). Now running **low-rank BLT (M≈U@V^T), r=256, from-scratch**, seed 42 (`run_blt_uv256_scratch_seed42.pt`) — past step 80,000/500,000 as of last check. See "Low-rank BLT (M = UV^T)" below for the implementation, verification, and why this is a from-scratch run rather than the originally-planned SVD-warm-start (Option 2).
...
- **`venus`**: running the **second sine-blend-schedule seed** (`run_gpt2_ema_blend75_sine_scratch_seed19.pt`, seed 19, GPT-2 small) — past step 90,000/500,000 as of last check, `effective_blend` tracking the sine formula exactly. The first sine seed (seed42) already finished and posted the **best LAMBADA acc in the whole EMA/blend family** (0.281) — see "GPT-2 medium" below for the full sine-schedule writeup. This seed19 result (expected in ~6 days from 2026-07-28) is the confirm-or-refute point for whether that was a real effect or favorable variance, and is also the trigger for a decision on the `io` medium run below. The three fixed-75/25-blend small-model seeds (42/19/7_v2) all finished earlier and are fully benchmarked — no longer in progress.
- **`io`**: running a **from-scratch sine-blend medium EMA run** (`run_gpt2_medium_ema_blend75_sine_seed42.pt`), launched 2026-07-28 to replace the original fixed-75/25-blend medium run, which was showing ~4x noisier in-training val_ppl than `bender`'s baseline at comparable training fraction (28% vs 6.5% relative swing). The **original fixed-blend checkpoint was paused, not deleted** — `run_gpt2_medium_ema_blend75_seed42.pt` sits safely at step 594,500 (val_ppl 46.81) and is fully resumable if the sine bet doesn't pan out once the venus seed19 result above lands. See "GPT-2 medium" below for the full decision writeup.
...
5. **UV^T (low-rank BLT)**: implemented and a from-scratch r=256 run is in progress on `titan`. See details below.
