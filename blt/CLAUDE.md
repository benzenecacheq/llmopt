...
## Active run
**GPT-2 (standard MHA) from-scratch OWT with EMA per-token loss weighting, seed 42**: started 2026-06-19. `run_gpt2_ema_seed42.pt/.log`, target 500K steps. Mirrors `run_gpt2_baseline_seed42.pt` exactly (from-scratch, OWT, seed 42, 500K steps) but adds `--ema-loss-weighting` (default `--ema-decay 0.99`) for a clean comparison. Tests whether upweighting persistently-hard tokens (self-referential approximation of Rho-1's excess-loss idea, see `project_loss_function_ideas.md`) improves LAMBADA/long-range benchmarks over the unweighted baseline without hurting OWT perplexity. The EMA loss-weighting feature itself was added on another machine (commit ceea6d4) and merged in cleanly. This run lives on the original machine, not `io` — unrelated to the `io` work below.
...
**CRITICAL — this is the second attempt.** The first attempt (identical command, default `--warmup-steps`) ran cleanly to step 16,830 (loss healthy, 7.4–7.85) then went NaN at step 16,840 with no warning, and **kept training for 1000+ more steps on NaN loss, repeatedly overwriting both the checkpoint and its `.bak` with all-NaN tensors** (verified via a throwaway script loading the checkpoint and checking `torch.isnan`/`isinf` on all 630 tensors — both files were 100% corrupted). Root cause: BLT's single shared `M` matrix is used by every layer, so one numerical blowup (likely fp16 forward-pass overflow — `GradScaler` only protects the backward/gradient path, not forward-pass overflow) corrupts the entire model in one backward pass, far more severe than the equivalent failure would be in standard MHA. Both corrupted checkpoint/log files were deleted before relaunching.

**Fix applied** (`train.py`, commit `87184e0`, already on `io` via `git pull`):
- A pre-backward guard in the training loop: `if not torch.isfinite(loss): log(...); continue` — skips any batch that produces non-finite loss before it reaches `.backward()`, preventing it from corrupting parameters.
- `save_checkpoint()` now calls a `model_is_finite(model)` check first and **raises `RuntimeError` instead of saving** if any parameter is NaN/Inf, so a bad state can never silently overwrite a good checkpoint (or its `.bak`).
- Verified via a direct unit test (forced NaN into a toy `nn.Linear`, confirmed detection and that no file gets written).

**Status as of hand-off (2026-06-22, ~14:21 local time on `io`)**: step **5,610** of 50,300, elapsed ~10,241s (~2.85h). Loss currently oscillating ~10-13 (healthy-ish for this stage). GPU 100% util, ~28.6GB memory, stable. No NaN/Inf in the log, no skip-guard triggers yet (file is clean — `grep -i "skip\|nan\|inf"` returns nothing) — both instability events below resolved without the safety net being needed.

**Instability events observed in *this* (second) run** — both self-recovered, neither involved NaN/Inf or guard triggers:
1. **Step ~470–630** (mid-warmup, LR still ramping ~1e-5–1.6e-5): loss climbed from ~8 to a peak of **26.5** at step 630, then steadily declined back to ~9.5-10 by step 1450. Took ~800 steps to fully recover.
2. **Step ~2200–4050** (post-warmup, full LR 5e-5): a slower, milder drift — loss climbed from ~10 to a peak of **17.1** at step 4050, then declined back to ~10-13 by step 4400-5610 (current). Milder than event 1 (peak 17 vs 26.5) and more gradual (drift, not a sharp spike).

Working interpretation: these look like the model settling into the warm-started architecture (M built from an average of Wq@Wk^T across 48 layers — a fairly drastic approximation at init) rather than a sign of impending failure — both resolved via normal gradient descent, no intervention needed. But two such events in the first ~5,600 steps suggests recurring bumps may be a real feature of this run, not a one-off; worth bearing in mind when reading intermediate loss values, and worth re-confirming the safety guards stay quiet (`grep -i "skip\|nan\|inf" run_blt_xl_warmstart_wikitext.log` should stay empty) at each check-in.

**Step-0 sanity baseline** (for context when reading early loss values): loss 9.03 at step 0 (val_ppl 17869) — well above pretrained GPT-2 XL's ~2.73 nats (ppl 15.30), but well below random-guessing loss of ln(50257) ≈ 10.83. This gap is the expected cost of the M-averaging warm-start surgery, not a bug.

**Time estimate**: based on early steady-state pacing (~1.7-1.8s/step) plus eval overhead (~1000-step WikiText eval adds ~40-60s; 5000-step LAMBADA eval not yet measured), expect roughly **24-25 hours total** wall-clock from the 2026-06-22 ~11:29 launch, i.e. completion sometime 2026-06-23 afternoon/evening. This estimate predates seeing two recovered instability events, which add some wall-clock cost without necessarily changing the total step count needed — treat as approximate.

**Next steps once this finishes**: compare final WikiText ppl (and ideally LAMBADA acc via `--lambada-eval-every 5000` checkpoints already being logged) against the pretrained GPT-2 XL baseline (ppl 15.30) and against the small-model BLT warm-start result for a sense of how the gap scales. Then, per the user's framing ("then we can see what to do next"), decide whether an MHA control fine-tune at this scale is warranted (straightforward — `train.py --baseline --pretrained gpt2-xl ...`) and whether GQA at this scale is worth the `build_gqa_model()` code changes needed to support a pretrained warm-start path.
