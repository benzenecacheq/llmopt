...
The key finding from our evaluation is that ground-truth rankings and faithfulness rankings disagree substantially. The clearest case is naive proportional truncation (naive_65pct): it scores 23.4 on ground-truth average, competitive with all compressed methods, but only 95.5 on perplexity faithfulness and 90.7 on embedding faithfulness — worse than semantic scoring methods on both. Conversely, our pre-RoPE method scores lower on ground truth (23.9) but higher on faithfulness (104.2 / 85.7).

The mechanism is task-specific. On NarrativeQA, naive truncation achieves 4.9 ground-truth vs. the full model's 5.5 — seemingly competitive — while pre-RoPE scoring achieves 5.7. But perplexity faithfulness for NarrativeQA tells a different story: kq_post_rope scores 110.8 while naive scores 48.7. Naive truncation is being evaluated on a very different effective context than the full model (it sees only the last 65% of a long novel), and the F1 metric happens to favor the shorter outputs that result. Perplexity faithfulness reveals that naive's outputs are actually far less likely under the full-context model.

This reversal — ground-truth appearing favorable while faithfulness reveals poor approximation — is the central motivation for our evaluation framework.
...
The headline finding: **on ground truth, all semantic methods (kq_post_rope, SnapKV, naive_65pct) appear within 1 point of each other. Faithfulness reveals a 9-point gap in perplexity (95.5 for naive vs. 104.2 for kq_post_rope).** Naive truncation is a significantly poorer approximation of full-context behavior than ground-truth scores suggest.

Embedding faithfulness tells a partially different story: naive_65pct scores 90.7, the highest of any method. This reflects that naive truncation preserves contiguous text, making output embeddings more similar to the full-context output in semantic vector space. Perplexity and embedding faithfulness thus capture complementary aspects: perplexity measures whether the output is plausible under the full model's distribution; embedding measures whether the semantic content is similar. kq_post_rope is the best semantic-scoring method on both.
