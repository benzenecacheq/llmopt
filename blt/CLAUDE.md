...
**Practical consequence for how results in this section should be interpreted going forward**: LAMBADA acc (and the other downstream benchmarks) should be read as the primary signal for the EMA/cumulative family, with OWT ppl demoted to a guardrail (catches genuine degeneracy — a model that's become incoherent on ordinary text) rather than a co-equal target being "traded away." The framing used throughout this section so far (e.g. "cumulative buys LAMBADA at an OWT-ppl cost") isn't wrong mechanically, but risks reading as if OWT ppl were an independent quality signal worth protecting for its own sake, when for this family it's largely just measuring the size of a deliberate, intended deviation. Worth carrying this framing into `paper_ema.md` once written.
