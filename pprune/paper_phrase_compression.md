...
Values in nats; lower is better. Bold marks the best among Naive and Streaming (SnapKV and PyramidKV excluded from bold competition — both are substantially worse). Streaming leads on 8 of 16 tasks and the overall average; Naive leads on 8. Both KV-pruning methods are an order of magnitude worse than either prompt-construction method.
...
**The two metrics sharply disagree on SnapKV and PyramidKV.** On KL faithfulness, Streaming (0.141) edges Naive (0.175) on average, while SnapKV-Press (1.157) and PyramidKV (1.394) are an order of magnitude worse. On F_out, PyramidKV (79.1%) and SnapKV-Press (78.5%) are the two leading methods — effectively tied and 10+ points above naive (68.6%) — while Streaming (61.2%) sits below naive. These reversals are not contradictory: §9 explains the mechanism that decouples F_out from KL for post-hoc eviction methods.
...
Values in nats; lower is better. Bold marks the best among Naive and Streaming. Naive leads on 12 of 16 tasks; Streaming leads on 4. SnapKV-Press (0.733) and PyramidKV (0.734) are similarly poor, both 5.5× worse than Naive (0.133).
...
The pattern is consistent across architectures. On Mistral, SnapKV-Press (83.7%) and PyramidKV (83.5%) are effectively tied on F_out — the same reversal as Llama relative to KL — while Streaming (61.0%) and Naive (60.0%) cluster together, confirming the KV-pruning F_out advantage holds across both models.
...
The synthetic ablation in §6.3 makes a concrete prediction for SnapKV. SnapKV's retained positions are globally scattered — an approximate realization of the `scattered` geometry in Table 2 — so the `evicted` row is its expected operating regime, and the `rerotated` row is what key re-rotation should produce. Table 2 shows rerotated KL at 0.17× of evicted for scattered positions. If positional displacement is the dominant failure mode, applying `KeyRerotationPress` to SnapKV — re-encoding retained keys at compact positions 0…M−1 after post-hoc eviction, with enriched value representations unchanged — should produce a comparably large improvement on full LongBench evaluation.

`snapkv_rerotated` tests this directly. Across all 16 LongBench tasks on Llama-3.1-8B:
...
**Selection strategy is irrelevant once positions are corrected.** Applying the same re-rotation to PyramidKV (`pyramidkv_rerotated`) produces KL values identical to SnapKV+rot at all 16 tasks and all three retention rates, to three decimal places. PyramidKV and SnapKV use entirely different per-layer budget formulas — uniform versus layer-indexed — yet they collapse to a single curve after position correction. This result rules out token selection as a meaningful contributor to KL faithfulness under post-hoc eviction: the entire gap between SnapKV and SnapKV-Select (§6.2, Table 1; 1.157 vs. 0.093) was driven by positional displacement, not by the difference between attention-score selection and prompt construction. Any scattered selection policy that re-rotates retained keys will reach the same KL.
...
**Streaming enrichment.** The re-rotation result for SnapKV does not alter the enrichment finding for near-contiguous selection. `streaming_rerotated` (mean KL 0.141) still outperforms its same-selection prompt-construction equivalent `streaming_select` (0.197) — KV pruning outperforms its own prompt-construction version for recency-biased selection because enrichment from the full-context prefill exceeds the small positional displacement cost of a single head-to-tail gap. The two channels operate independently: enrichment benefits near-contiguous selection; position correction benefits scattered selection. For scattered selection with re-rotation, both benefits apply simultaneously, producing SnapKV+rot's outperformance over all other methods on KL.
...
The Mistral rankings replicate the Llama pattern. SnapKV+rot leads at both rates (0.030 at 65%, 0.096 at 35%), outperforming every prompt-construction method by the same wide margin as on Llama. Among prompt-construction methods, phr128 leads (0.121 at 65%, 0.159 at 35%). Unlike Llama, Streaming (0.161) trails Naive (0.133) on Mistral at every rate — Naive leads on 12 of 16 tasks (§5.5). SnapKV and PyramidKV show the same budget-tightening improvement (SnapKV 0.733→0.588, Pyr 0.734→0.586), and both remain far behind SnapKV+rot at every rate. F_out for Mistral SnapKV+rot is pending the Mistral GT re-run.
...
**Mistral-7B-v0.3** (65% and 35% retention; †SnapKV+rot F_out pending Mistral GT run).

| Retention | Naive | phr128 | Streaming | SnapKV | Pyr |
|---|---|---|---|---|---|
| 65% | 61.1 | 60.6 | 61.0 | 83.7 | **83.9** |
| 35% | 53.5 | 53.9 | 53.9 | 74.0 | **74.8** |

Mistral shows the same qualitative pattern as Llama: PyramidKV leads F_out at both rates (83.9%, 74.8%); SnapKV (kvpress) is competitive at 65% (83.7) and degrades to 74.0 at 35%; Streaming and prompt-construction methods cluster tightly (61.0 / 53.9 vs. Naive 61.1 / 53.5 at 65% / 35%). SnapKV+rot F_out will be added once the Mistral GT run completes.
...
**Tables C1a–C1c. Ground-truth accuracy per task, Llama-3.1-8B (higher is better). Bold = highest value per row. Streaming = streaming_rerotated (corrected re-run).**

**Table C1a. 65% retention.**

| Task             | Naive    | phr128   | Streaming | SnapKV   | Pyr      |
|------------------|----------|----------|-----------|----------|----------|
| NarrativeQA†     | 4.9      | 4.3      | **5.8**   | 5.6      | 5.5      |
| Qasper†          | 10.2     | 10.3     | 9.5       | 11.3     | **11.8** |
| MultifieldQA     | 27.0     | 25.3     | 26.3      | **30.2** | 29.4     |
| HotpotQA†        | 9.6      | 9.2      | 9.0       | 9.9      | **10.2** |
| 2WikiMQA†        | 12.6     | 10.7     | **13.9**  | **13.9** | 13.7     |
| MuSiQue†         | 7.0      | 6.1      | 5.6       | **7.2**  | 7.1      |
| GovReport        | 19.8     | 19.7     | 18.9      | 19.6     | **20.1** |
| QMSum†           | **11.5** | 9.3      | 9.9       | 9.7      | 9.4      |
| MultiNews        | 16.5     | 16.4     | 17.7      | **18.1** | 17.9     |
| TREC             | 66.0     | 70.0     | 68.0      | **71.0** | **71.0** |
| TriviaQA         | 17.3     | 17.2     | 17.4      | **17.5** | **17.5** |
| SAMSum           | 16.5     | 16.5     | **17.1**  | 16.2     | 16.3     |
| PassageCount†    | 1.0      | 2.0      | **3.0**   | **3.0**  | **3.0**  |
| PassageRetrieval | 37.0     | 29.0     | 37.0      | **44.0** | **44.0** |
| LCC              | 63.4     | 61.3     | 67.2      | 68.1     | **68.5** |
| RepoBench-P      | 53.9     | 50.7     | 53.2      | 55.4     | **56.0** |
| **Average**      | 23.4     | 22.4     | 23.7      | 25.0     | **25.1** |

**Table C1b. 50% retention.**

| Task             | Naive    | phr128   | Streaming | SnapKV   | Pyr      |
|------------------|----------|----------|-----------|----------|----------|
| NarrativeQA†     | 5.7      | 4.2      | 5.5       | 6.7      | **6.8**  |
| Qasper†          | 8.6      | 9.9      | 9.4       | **11.1** | 9.8      |
| MultifieldQA     | 23.0     | 24.8     | 25.4      | 29.2     | **29.5** |
| HotpotQA†        | 8.9      | 9.2      | **9.9**   | 9.7      | 9.6      |
| 2WikiMQA†        | 12.7     | 12.9     | 13.8      | **14.0** | 13.8     |
| MuSiQue†         | 5.8      | 6.7      | 5.3       | **7.0**  | 6.8      |
| GovReport        | 19.5     | 18.5     | 19.0      | **20.0** | 19.5     |
| QMSum†           | 9.5      | 9.0      | 9.1       | **9.7**  | **9.7**  |
| MultiNews        | 17.5     | 15.9     | **17.6**  | 16.1     | 15.4     |
| TREC             | 63.0     | 65.0     | 67.0      | **71.0** | 68.0     |
| TriviaQA         | 17.4     | 16.8     | **17.6**  | 17.4     | 17.3     |
| SAMSum           | **17.7** | 16.8     | 17.6      | 16.1     | 17.6     |
| PassageCount†    | 2.0      | 2.0      | **3.0**   | **3.0**  | **3.0**  |
| PassageRetrieval | 21.0     | 27.0     | 36.0      | **44.0** | 43.0     |
| LCC              | 61.6     | 62.5     | 66.8      | **68.1** | 63.9     |
| RepoBench-P      | 51.4     | 48.7     | 53.8      | **55.7** | 52.8     |
| **Average**      | 21.6     | 21.9     | 23.6      | **24.9** | 24.2     |

**Table C1c. 35% retention.**

| Task             | Naive    | phr128   | Streaming | SnapKV   | Pyr      |
|------------------|----------|----------|-----------|----------|----------|
| NarrativeQA†     | **7.8**  | 3.9      | 6.5       | 6.4      | 6.4      |
| Qasper†          | 7.7      | 9.0      | 7.8       | **10.8** | **10.8** |
| MultifieldQA     | 21.2     | 24.0     | 24.2      | **30.2** | 30.0     |
| HotpotQA†        | **11.4** | 10.0     | 9.4       | 10.0     | 10.0     |
| 2WikiMQA†        | 11.0     | 11.3     | 12.5      | **14.6** | **14.6** |
| MuSiQue†         | 4.7      | 6.1      | 5.2       | **6.5**  | **6.5**  |
| GovReport        | 18.7     | **19.1** | 18.6      | 18.9     | 19.0     |
| QMSum†           | 9.9      | 10.0     | 9.9       | **10.2** | **10.2** |
| MultiNews        | **16.7** | 15.3     | 16.0      | 15.7     | 15.8     |
| TREC             | 60.0     | 68.0     | 63.0      | **69.0** | **69.0** |
| TriviaQA         | 17.3     | 16.8     | **18.3**  | 17.4     | 17.4     |
| SAMSum           | 17.6     | 17.8     | **18.2**  | 17.3     | 17.3     |
| PassageCount†    | **3.0**  | **3.0**  | **3.0**   | **3.0**  | **3.0**  |
| PassageRetrieval | 12.0     | 19.0     | 37.0      | **44.0** | **44.0** |
| LCC              | 62.4     | 60.9     | 66.2      | **67.0** | **67.0** |
| RepoBench-P      | 50.6     | 49.2     | **54.0**  | **55.1** | **55.1** |
| **Average**      | 20.8     | 21.5     | 23.1      | **24.8** | **24.8** |

**Tables C2a–C2b. Ground-truth accuracy per task, Mistral-7B-v0.3 (higher is better). Bold = highest value per row. Streaming = streaming_rerotated (corrected re-run).**

**Table C2a. 65% retention.**

| Task             | Naive    | phr128   | Streaming | SnapKV   | Pyr      |
|------------------|----------|----------|-----------|----------|----------|
| NarrativeQA†     | 2.7      | **5.5**  | 5.4       | 5.1      | 4.1      |
| Qasper†          | 4.3      | 4.2      | 5.0       | **5.1**  | 4.8      |
| MultifieldQA     | 22.1     | 22.7     | 23.2      | **24.9** | 24.8     |
| HotpotQA†        | 10.6     | 9.9      | 9.8       | **10.7** | 10.6     |
| 2WikiMQA†        | 9.8      | **12.2** | 11.3      | 11.7     | 11.7     |
| MuSiQue†         | 3.9      | 4.7      | 4.4       | **5.1**  | **5.1**  |
| GovReport        | 19.7     | 19.5     | 19.4      | 19.8     | **20.0** |
| QMSum†           | **8.4**  | 8.1      | 8.3       | 7.9      | 7.9      |
| MultiNews        | 17.2     | 16.9     | 17.1      | 17.2     | **17.3** |
| TREC             | 67.0     | 67.0     | 70.0      | 70.0     | **71.0** |
| TriviaQA         | 24.2     | 24.5     | **26.5**  | 23.4     | 23.3     |
| SAMSum           | 17.8     | 17.0     | **18.3**  | 17.9     | 18.1     |
| PassageCount†    | **3.0**  | **3.0**  | 1.0       | 1.0      | 1.0      |
| PassageRetrieval | 26.0     | 24.0     | 30.0      | **39.0** | **39.0** |
| LCC              | 60.2     | **66.9** | 62.1      | 63.2     | 63.2     |
| RepoBench-P      | **54.4** | 52.0     | 54.1      | 54.1     | 54.0     |
| **Average**      | 22.0     | 22.4     | **22.9**  | **23.5** | **23.5** |

**Table C2b. 35% retention.**

| Task             | Naive    | phr128   | Streaming | SnapKV   | Pyr      |
|------------------|----------|----------|-----------|----------|----------|
| NarrativeQA†     | 3.9      | 2.9      | 4.7       | **5.1**  | **5.1**  |
| Qasper†          | 4.0      | 4.3      | 4.2       | **4.9**  | 4.8      |
| MultifieldQA     | 18.2     | 22.0     | 18.6      | **25.1** | **25.1** |
| HotpotQA†        | 11.0     | 10.4     | **11.3**  | 10.4     | 10.4     |
| 2WikiMQA†        | 11.6     | 12.2     | **12.4**  | 11.2     | 11.2     |
| MuSiQue†         | 4.6      | 5.0      | 3.5       | **5.2**  | **5.2**  |
| GovReport        | **18.6** | 18.4     | 18.0      | **18.6** | **18.6** |
| QMSum†           | 7.8      | **8.2**  | **8.2**   | 7.8      | 7.8      |
| MultiNews        | **16.5** | 15.6     | 15.2      | 15.7     | 15.7     |
| TREC             | 60.0     | 65.0     | 61.0      | **66.0** | **66.0** |
| TriviaQA         | 25.3     | 22.4     | **26.9**  | 23.7     | 23.7     |
| SAMSum           | 17.2     | 15.3     | **18.3**  | 17.8     | 17.8     |
| PassageCount†    | **3.0**  | 1.0      | 1.0       | 1.0      | 1.0      |
| PassageRetrieval | 15.0     | 25.0     | 22.0      | **39.0** | **39.0** |
| LCC              | 57.4     | **65.4** | 62.6      | 62.3     | 62.3     |
| RepoBench-P      | 49.5     | 46.7     | 52.1      | **53.7** | **53.7** |
| **Average**      | 20.2     | 21.2     | 21.2      | **23.0** | **23.0** |
