...
These results exhibit some interesting characteristics that aren't completely obvious or in some cases, very easy to explain.
...
- **Block_end geometry: positional shift and structural corruption both contribute.** Even without enrichment, the gapped presentation (2.373) is 13.4× worse than gapless (0.177) — a substantial gap that is perhaps surprising given that the retained block has only one causal gap and is otherwise contiguous. Enrichment increases the damage further: evicted (3.493, 19.8×) exceeds gapped (2.373, 13.4×). Most notably, re-rotation — which fully recovers the scattered geometry — provides only a modest improvement here: rerotated (2.814, 15.9×) does not recover to the no-enrichment gapped baseline, let alone to gapless. One plausible explanation is that the gapped damage arises from the positional offset of the tail block itself — keys at positions ~1400–4000 rather than 0–2600, even without any gap in the block, may create a mismatch the model was not trained to handle. Enrichment may then add a separate cost, with value representations shaped by the now-evicted head persisting in the cache. Re-rotation corrects the key positions but leaves value representations unchanged, which could explain why it improves on evicted but does not recover to gapless.  We didn't test this theory further and there are doubtless other equally plausible explanations .
...
PyramidKV's per-layer budget variation introduces a structural complication that SnapKV does not face. Right-aligned re-rotation assigns each layer's retained keys to `[T − n_kept, T)`, but because n_kept differs across layers under the pyramid budget, queries and keys at different layers operate on different absolute-position ranges simultaneously. This cross-layer positional disparity is most severe when the pyramid is steepest — at 50% retention, n_kept spans from ≈0.66T at the bottom layer to just the `window_size` floor at the top, producing the widest per-layer mismatch. The 50% gap in Pyr+rot therefore likely reflects a combination of information loss at aggressively pruned upper layers and residual positional disparity that right-aligned re-rotation cannot eliminate. In deeper models, where the pyramid budget is distributed across more layers, this disparity may be more pronounced.
