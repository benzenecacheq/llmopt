...
- **`bender`** (local): GPT-2 **medium** (355M, 24 layers/16 heads/1024 dim) baseline from-scratch OWT, seed 42, running — `run_gpt2_medium_baseline_seed42.pt`. Target extended to **1.5M steps** (resumed at step 21,000 with `--max-steps 1500000`, cosine schedule correctly re-stretched, not restarted — see "GPT-2 medium" below for the full story: motivation, the model-size code changes, the OOM/batch-size fix, and the thermal-throttling timing analysis).
...
- **`io`**: 32GB V100, not overheating (unlike `bender`), currently busy with something else (not this project). Would be a better home for the GPT-2-medium run once free — see below.
...
**Plan: `io` will run the EMA counterpart once free.** User clarified after some cross-talk: `bender`'s medium run has no `--ema-loss-weighting`/`--ema-blend` flags (confirmed via live `ps`) — it's the plain non-EMA baseline, not "the EMA one" as briefly assumed. So `io` is slated to run the matching **EMA 75/25-blend** version (`--ema-loss-weighting --ema-blend 0.75`, matching the established small-model protocol), same seed (42), same `--dataset openwebtext_large`, same 1.5M-step target, `--batch-size 4` with no grad-accum (32GB should fit true batch=4 in one pass, avoiding the small-batch inefficiency tax `bender` needed to work around). **Not launched yet — user said to wait for explicit go-ahead.** `io`'s existing repo checkouts are "in a state of flux" (other unrelated work in progress there) — user explicitly asked for a **fresh clone into a new directory** rather than reusing/pulling the existing checkout, to avoid disturbing whatever else is in progress on that machine.
