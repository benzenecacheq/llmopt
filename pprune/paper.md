...
Evaluated on LongBench across retrieval, summarization, multi-document QA, and code tasks with Llama-3.1-8B at 65% retention, our method recovers 86% of full-context performance and outperforms naive truncation on retrieval by 9 points (27% vs 18%). Results break down diagnostically by task type: summarization shows the largest gap versus recency-only baselines, retrieval benefits most from position-independent scoring, and code completion reveals a residual weakness where locality matters more than semantic relevance. Two targeted fixes — entropy-based head budget allocation and K-std adaptive decay — both fail to improve code completion, converging on a diagnosis: the failure is a within-head token-selection problem that requires either explicit task detection or a scoring signal rewarding syntactic continuity. We validate on Mistral-7B-v0.3 with identical hyperparameters (93% retention), and implement the method as a KVPress press subclass, achieving a 10% generation speedup over full context.
...
**KV cache eviction.** H2O [Zhang et al., 2023] identifies "heavy hitter" tokens via accumulated attention scores and evicts the rest. SnapKV [Li et al., 2024] selects tokens by pooling attention weights from the last several queries over all key positions, using post-RoPE vectors. Our method differs in two ways: we use pre-RoPE vectors for scoring (removing positional bias) and combine the alignment signal additively with V-norm. We implement and benchmark SnapKV directly, finding that pre-RoPE scoring outperforms SnapKV by up to 13 points on retrieval tasks. However, the attention-based scoring in SnapKV has a task-specific advantage for code completion (§5.3): the model's learned attention patterns implicitly capture syntactic dependencies that explicit feature signals do not, and at 40% retention SnapKV retains 101% of full-context code performance compared to 94% for our method. SnapKV-style methods also have a practical limitation at long contexts: attention weights are not materialized by Flash Attention kernels, requiring a separate computation pass; our method requires only K and V vectors and is therefore directly compatible with Flash Attention. StreamingLLM [Xiao et al., 2023] always retains "attention sink" tokens (typically the first few tokens) plus a recent window, making it a strong baseline on recency-dominated tasks.

**Value-norm scoring.** VATP [EMNLP 2024] proposes scoring tokens by the product of their attention weight and L1 value norm, observing that attention sinks receive high attention but near-zero V-norm. Our method uses V-norm as a standalone component and finds it sufficient for synthetic retrieval but insufficient for natural-language QA. Feng et al. [2025] provide a theoretical justification for value-norm-based scoring by deriving an upper bound on output perturbation; their bound depends on the L1 norm of the *projected* value state V·W^O rather than the raw value vector. They propose a two-stage selector applied on top of SnapKV or AdaKV: top-α tokens by attention weight, remainder by (A+ε)⊙‖V·W^O‖₁. Their primary evaluation targets aggressive compression (20–40% retention); at 40% retention on Llama-3.1-8B they report domain-averaged scores within 1–2% of full context across QA, summarization, and code domains. A direct numerical comparison with our method at 65% retention is not straightforward because the operating regimes differ and they report domain aggregates rather than per-task scores. Our work differs in two respects: we use raw V-norm without the output projection (motivated empirically rather than theoretically), and we combine it with pre-RoPE KQ alignment rather than post-RoPE attention weights. A per-task analysis of where V-norm succeeds and fails is presented in §5.3.
...
Code completion regresses relative to both full context and naive truncation (LCC: 57.4 pruned vs 68.1 full vs 66.5 naive; RepoBench-P: 49.0 vs 55.6 vs 54.7). This is a diagnostic finding: it tells us precisely what our scoring function does not model. The hypothesis that scoring fails due to a budget or decay problem is tested in §5.8 and ruled out. Here we pursue a deeper diagnosis.

**V-norm diagnostic study.** To understand the failure mechanistically, we instrumented the scorer to record raw V-norm (the L2 magnitude of each token's value vector) and KQ alignment for every token, layer, and head across five task types: LCC, HotpotQA, GovReport, PassageRetrieval, and 2WikiMQA (5 examples each, Llama-3.1-8B, 32 layers). Aggregating over all layers and heads, the global V-norm distributions are similar across tasks (median 3.07–3.19, std 1.71–1.72 for all tasks). But the character of the low-V-norm tail differs sharply.

In QA and summarization contexts, the bottom quartile of V-norm is dominated by semantically vacuous tokens: isolated spaces, function words (`The`, `a`), and repeated padding. These tokens are genuinely unimportant — discarding them causes no information loss. In code contexts, the bottom quartile is occupied by structurally critical tokens: opening parentheses `(`, indentation (`\t`, `\t\t\t\t`), and common keywords (`void`, `new`, `is`). These tokens carry minimal semantic content — the model has seen millions of them and nothing surprising happens in their value vectors — but they are syntactically load-bearing. Removing a `(` or a `void` from a function signature may break the completion.

Crucially, KQ alignment for these structural code tokens is mid-range (~0.5–0.7 after normalization), not low. They are not semantically irrelevant from a query perspective; they simply compete against tokens with both higher V-norm and comparable KQ alignment (domain-specific identifiers and method names), and lose. We tested whether increasing α from 0.65 to 0.85 — reducing the V-norm term's influence — rescues structural tokens. It does not (LCC: 25.8 vs 25.9, within noise), confirming that both signals rank structural tokens below content tokens and no reweighting can easily recover them.

The V-norm std is slightly lower for LCC (0.667) than for all other tasks (0.682–0.707), consistent with this picture: code lacks the high-V-norm "spike" tokens (genuinely surprising or unusual words) that drive the upper tail in QA/summarization. The distribution is compressed toward the middle, where structural and content tokens are interleaved without a clean separating signal.

This analysis sharpens the diagnosis: the code regression is not a recency-bias problem, a budget-distribution problem, or a head-allocation problem. It is a signal-space problem. Neither V-norm nor KQ alignment is designed to capture syntactic necessity, and the combination of the two is no exception.

**What SnapKV gets right for code.** The V-norm diagnostic points toward why attention-based scoring (SnapKV) fares better on code than our feature-based approach. SnapKV pools post-RoPE attention weights from the last `window_size` queries, asking: "what did the model *actually attend to* while processing the recent context?" For code, the model's learned attention patterns are already syntactically aware — through training, the model has learned to attend to the `(` that opens the current function call, the variable declaration it is resolving, and the scope-defining brackets it is tracking. These syntactic dependencies appear directly in the attention weights without any explicit feature engineering. Our pre-RoPE KQ alignment asks a different question: "which key vectors are semantically similar to the recent queries?" For prose, semantic similarity and importance are well-aligned. For code, they are not: a bracket at position 5000 is attended to because it is syntactically load-bearing, not because it is semantically similar to anything.

This difference is quantifiable. Running our additive scorer at the same 40% retention used in Feng et al. [2025] and comparing retention ratios (score / full-context score): our method retains **94%** of full-context LCC performance at 40% retention, while Feng et al. report SnapKV retaining **101%** on their Code domain at the same retention level. The 7-point gap isolates the contribution of attention-based vs. feature-based scoring for code, holding compression ratio fixed.

The positional bias we remove with pre-RoPE scoring is, for code, actually *useful*: post-RoPE attention has a natural recency weighting, and for code completion the most syntactically relevant tokens tend to be nearby — the current function body, recent declarations, the immediately enclosing scope. Our pre-RoPE correction removes exactly the positional information that helps here.

A genuine fix would require either an explicit syntactic signal — attention sink detection, token-type priors, or a learned importance head — or task detection to switch to an attention-based scorer for code inputs.
...
Head-aware allocation matches the global-budget additive scorer within noise on every task, including LCC. We additionally tested two adaptive decay variants aimed specifically at the code completion gap.

**K-std adaptive decay.** Per-head key-vector standard deviation is used to detect homogeneous (code-like) contexts and apply stronger recency bias. This fails to improve LCC (24.5 vs 25.8 additive). Root cause: code contexts have high K-std — diverse token types (identifiers, operators, string literals, brackets) span a wide representation space — so K-std does not distinguish code from QA, and the signal fires in the wrong direction.

**Content-entropy adaptive decay.** Per-head entropy of content-only scores (α·kq + (1−α)·vn, no decay term) is used as a scorer-confidence signal: high entropy → uncertain scorer → apply stronger recency bias. We parameterize the adjustment as min\_decay\_i = min\_decay\_semantic × (1 − γ · entropy\_norm\_i), with γ=0.5 as a middle-of-the-road coefficient. Results (30 examples, 65% retention):

| Task | Full | Additive | Entropy-Adaptive |
|---|---|---|---|
| PassageRetrieval | 13.3 | 16.7 | 3.3 |
| HotpotQA | 9.7 | 8.7 | 6.7 |
| 2WikiMQA | 16.0 | 15.9 | 12.4 |
| GovReport | 17.0 | **16.9** | 12.8 |
| QMSum | 13.8 | 12.4 | 6.6 |
| LCC | **25.9** | 25.8 | 23.3 |

Entropy-adaptive decay is worse on every task, with large drops on passage retrieval (16.7→3.3) and summarization (12.4→6.6). The adaptive mechanism that was intended to rescue code completion is actively harming tasks that were already working. The failure mode is interpretable: high content-score entropy in retrieval and summarization heads signals "many tokens are informative," not "the scorer is uncertain" — reducing min\_decay there (adding recency bias) discards the long-range tokens those heads need most.

All three ablations converge on the same diagnosis: the LCC gap is a within-head token-selection problem, not a budget, decay-parameter, or per-head allocation problem. The scoring function itself ranks syntactically necessary but content-unremarkable tokens (brackets, keywords, identifiers) below prose tokens that dominate the semantic signal. Addressing this directly — via task-conditioned scoring or a signal that rewards syntactic continuity — is a direction for future work.
