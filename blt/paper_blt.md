...
We also tested a hybrid architecture — half standard MHA layers, half BLT layers — which recovers most of what full BLT gives up in quality, at the cost of keeping only half of BLT's memory benefit.  A low-rank variant of BLT (M ≈ UV^T, motivated by cache residency and tensor-parallel compatibility at larger model sizes) trains from scratch to beat full-rank BLT outright, a result confirmed across three seeds, while nearly matching standard MHA on zero-shot benchmarks at roughly half the attention parameters and 32% faster training on identical hardware.  Extending this factorization to multiple (U,V) pairs isolates which of BLT's two sharing constraints costs more: relaxing the constraint across attention heads recovers about half of the remaining gap to standard MHA, while relaxing the analogous constraint across layers does not help at all, and is in fact the single worst-performing variant of BLT tested in this paper.
...
BLT trails both baselines by a consistent ~0.10 nat / ~3 ppl margin; GQA is marginally ahead even of standard MHA on this one metric.  This OWT result is backed by multi-seed evidence: three independent BLT seeds (42, 19, 7) converge to 31.05, 30.48, and 30.81 ppl respectively — a narrow ~0.6 ppl band, well inside the gap to either baseline.  GPT-2 and GQA are each reported from a single seed.
...
**LAMBADA (0.225 GPT-2 / 0.204 GQA / 0.212 BLT).** GQA's point estimate trails GPT-2 by a statistically meaningful margin (≈2.6 combined SE); BLT's gap to GPT-2 points the same direction but is smaller and not significant at conventional thresholds (≈1.7 SE), and BLT vs. GQA itself is within noise (≈1.0 SE) at this sample size.  GQA retains per-layer W_k matrices but reduces each layer's effective key rank from 768 dimensions (12 heads × 64) to 128 (2 groups × 64) — a 6× compression.  That this compression appears to hurt LAMBADA about as much as BLT's cross-layer sharing does — even though only GQA's gap to GPT-2 is individually significant — suggests that LAMBADA specifically requires high per-layer key capacity, not merely per-layer key matrices.  Reduced rank within a layer (GQA) and a single shared matrix across layers (BLT) are both plausible contributors to degraded long-range prediction, though GQA's is the one confirmed statistically.
...
We placed the 6 unconstrained MHA layers first and the 6 shared-M BLT layers last, rather than the reverse, based on evidence that later transformer layers are less critical to model quality than early ones: Gromov et al. show that removing 20–55% of the deepest layers in pretrained LLMs causes minimal performance degradation after light fine-tuning, while removing early layers is far more damaging ([arXiv 2403.17887](https://arxiv.org/abs/2403.17887), ICLR 2025).  If later layers are less critical, they are also the more natural place to absorb BLT's expressiveness constraint — Section 7.2 generalizes this layer-ordering intuition into a graduated, multi-block sharing scheme.
...
### 4.7 Low-Rank BLT (UV^T): A Three-Seed Confirmed Result
...
We trained GPT-2-scale BLT with M replaced by UV^T at r=256 (81% of a converged full M's Frobenius energy), from scratch, three seeds (42, 19, 7), using the identical 500K-step OWT protocol as every other from-scratch run in this paper (Section 4.3).

| Metric | UV256 seed 42 | UV256 seed 19 | UV256 seed 7 | BLT seed 42 (500K) | BLT seed 19 (500K) | BLT seed 7 (500K) | GPT-2 seed 42 (500K) |
|---|---|---|---|---|---|---|---|
| OWT held-out ppl | 29.97 | 30.13 | 30.38 | 31.69 | 32.87 | 30.81 | 27.78 |
| OWT held-out loss | 3.4002 | 3.4054 | 3.4137 | 3.4561 | 3.4927 | 3.4279 | 3.3243 |
| LAMBADA acc | 0.213 | 0.214 | 0.207 | 0.188 | 0.208 | 0.212 | 0.225 |
| LAMBADA ppl | 222.8 | 237.9 | 265.7 | 369.1 | 343.4 | 244.4 | 174.6 |
| HellaSwag acc_norm | 0.269 | 0.267 | 0.267 | 0.267 | 0.271 | 0.268 | 0.268 |
| PIQA acc_norm | 0.569 | 0.583 | 0.578 | 0.567 | 0.559 | 0.568 | 0.579 |
| Winogrande acc | 0.512 | 0.492 | 0.502 | 0.489 | 0.500 | 0.516 | 0.505 |
...
**Confirmed across three seeds.**  Full-rank BLT's own three seeds span 2.06 ppl on OWT held-out perplexity (30.81 to 32.87) from seed variation alone — a range that could have swallowed UV256's improvement over BLT's best individual seed if seed 42 had simply been a favorable draw.  It didn't: all three UV256 seeds land in a tight 29.97–30.38 band, sitting entirely below all three full-rank BLT seeds' 30.81–32.87 range with no overlap at all.  This is now the same three-seed standard as every other from-scratch result in this paper, not a preliminary read: **low-rank BLT (r=256, 66% of full M's parameters) matches or beats full-rank BLT on the primary metric while also fixing the tensor-parallelism problem** (Section 6) — genuinely the better BLT variant, not just a cheaper one.

**UV256 versus standard MHA, not just versus full-rank BLT.**  The comparison above is internal to the BLT family; a more consequential question is what UV256 costs and saves relative to standard multi-head attention itself.  At r=256, UV256's total attention parameters (per-layer W_v, W_o, plus one globally-shared U, V pair) are 14.5M, versus 28.3M for standard MHA's twelve independent per-layer W_q/W_k/W_v/W_o sets — a 48.6% reduction, matching full BLT's 48% almost exactly (Section 6) despite UV256 keeping a genuine, factored Q/K computation rather than fusing it into one opaque matrix.  Against the three-seed MHA baseline from Section 4.3:

| Metric | UV256 seed 42 | UV256 seed 19 | UV256 seed 7 | GPT-2 (MHA) seed 42 | GPT-2 (MHA) seed 19 | GPT-2 (MHA) seed 7 |
|---|---|---|---|---|---|---|
| OWT held-out ppl | 29.97 | 30.13 | 30.38 | 27.78 | 28.24 | 27.36 |
| LAMBADA acc | 0.213 | 0.214 | 0.207 | 0.225 | 0.208 | 0.214 |
| HellaSwag acc_norm | 0.269 | 0.267 | 0.267 | 0.268 | 0.266 | 0.270 |
| PIQA acc_norm | 0.569 | 0.583 | 0.578 | 0.579 | 0.572 | 0.583 |
| Winogrande acc | 0.512 | 0.492 | 0.502 | 0.505 | 0.520 | 0.519 |

OWT held-out ppl shows a real, consistent gap (~30.2 vs. ~27.8, matching the ~0.10 nat cost documented for full-rank BLT throughout this paper) — collapsing twelve per-head Q/K projections to one shared pair is not entirely free.  But LAMBADA acc is close to indistinguishable (UV256's three-seed mean 0.211 vs. MHA's three-seed mean 0.216, well inside seed noise for either group), and HellaSwag/PIQA/Winogrande show no consistent direction either way.  A model using roughly half the attention parameters and, per the same-hardware measurement below, meaningfully less attention compute reproduces standard MHA's downstream task performance on every benchmark except the one metric (next-token OWT loss) where BLT-family models have always paid a cost.

**A same-hardware speed measurement.**  Isolating architecture from machine (both runs on the `titan` P100, no cross-machine confound): standard MHA trains at 1,052 ms/step (526,162s / 500,000 steps, seed 19), UV256 at 718 ms/step (358,797s / 500,000 steps, seed 42) — UV256 is **32% faster**, a considerably larger same-hardware speedup than full-rank BLT's own ~7.7% edge over MHA measured on a V100 (Section 3).  This tracks the architecture directly: standard multi-head attention computes twelve independent (L,L) score matrices and twelve softmaxes per layer; UV256 (like full-rank BLT) computes exactly one of each, applied across the entire D-dimensional value vector.  The FLOPs and memory-parameter savings from head-collapse compound in a way the cross-layer-sharing savings alone do not.

**Reframing the "which axis is redundant" question — a first, indirect look.**  Section 4.5's hybrid experiment showed that restoring six layers' worth of *cross-layer* independence (full per-head MHA layers interleaved with full BLT layers) recovers over 75% of BLT's OWT-loss gap.  UV256's result here isolates the other axis: collapsing *within-layer* head diversity to a single shared (and rank-limited) Q/K computation costs almost nothing on four of five benchmarks, while still paying BLT's usual OWT-loss tax.  Read together, these two results point the same way — the redundancy this paper set out to find is concentrated in **within-layer head multiplicity**, more than in **cross-layer specialization**.  But the comparison is indirect: the hybrid changes *which* layers are unconstrained by swapping in real MHA layers (relaxing both axes at once for those six layers), while UV256 only ever tests a single (U,V) pair against the zero-(U,V)-groups baseline.  Section 4.8 tests both axes directly, using a matched capacity budget on each, and confirms this reading with much sharper evidence.

### 4.8 Isolating the Head Axis from the Layer Axis

Section 7.2 frames BLT's design space as two *independent* axes — how many distinct sharing groups to use across heads, versus across layers — and proposes a "fuller sweep along either axis" as future work.  This section reports the first such sweep: one point on each axis, built with the smallest possible change to each variant so the comparison isolates the axis rather than the mechanism.

**Head axis: `num_uv_groups=4`.**  A direct extension of Section 4.7's low-rank BLT: instead of one globally-shared (U, V) pair at rank r=256, four independent (U, V) pairs, each still rank 256 and each still shared globally across all 12 layers.  Heads are partitioned into 4 groups of 3; each group's queries and keys come from its own (U, V) pair, and W_v/W_o are split per group in the same style as GQA's value grouping.  This relaxes only the *within-layer* head-collapse constraint — cross-layer sharing is untouched, since every (U, V) pair still serves the whole depth of the network.

**Layer axis: 3 distinct full-rank M's, strided across layers (`layers-per-m=4`).**  The natural analogue on the other axis, built from full-rank BLT rather than the low-rank variant: instead of one D×D matrix M shared by all 12 layers, three matrices M₀, M₁, M₂, with layer *i* using M_{i mod 3}.  The assignment is strided, not contiguous — M₀ serves layers {0, 3, 6, 9}, spanning the network's full depth — so this tests only whether relaxing cross-layer sharing *at all* helps, not whether early or late layers specifically need more capacity (that is the hybrid's question, Section 4.5).  Every head within a layer still shares that layer's single M, so the head-collapse constraint is untouched.

These two variants add the *same* amount of extra capacity relative to their respective single-shared baselines — three extra (U,V) pairs at 393,216 parameters each, or two extra full M's at 589,824 parameters each, both totaling exactly 1,179,648 additional parameters — even though the underlying mechanism differs (thin rank-256 pairs vs. full D×D matrices) and the starting baselines are themselves slightly different sizes (110,658,816 for single-pair UV256 vs. 110,855,424 for full-rank BLT).  If anything this works against the head-axis variant: full-rank M's carry more expressive capacity per matrix than a rank-256 (U,V) pair, so the layer-axis variant is not disadvantaged by capacity — only by which axis it relaxes.

Both trained from scratch on OWT for the standard 500K-step protocol (Section 4.3); `num_uv_groups=4` across three seeds, `layers-per-m=4` for one seed (the effect size below made a confirming second seed low-priority, though it has not been run):

| Metric | `num_uv_groups=4` seed 42 | `num_uv_groups=4` seed 19 | `num_uv_groups=4` seed 7 | `layers-per-m=4` seed 42 | UV256 single-pair (3 seeds) | Full-rank BLT (3 seeds) | GPT-2 (MHA, 3 seeds) |
|---|---|---|---|---|---|---|---|
| OWT held-out ppl | 28.95 | 28.95 | **28.85** | 33.14 | 29.97–30.38 | 30.81–32.87 | 27.36–28.24 |
| LAMBADA acc | 0.2045 | 0.2016 | 0.2073 | 0.1976 | 0.207–0.214 | 0.188–0.212 | 0.208–0.225 |
| HellaSwag acc_norm | 0.2677 | 0.2683 | 0.2652 | 0.2678 | 0.267–0.269 | 0.267–0.271 | 0.266–0.270 |
| PIQA acc_norm | 0.5686 | 0.5637 | 0.5686 | 0.5626 | 0.569–0.583 | 0.559–0.568 | 0.572–0.583 |
| Winogrande acc | 0.4988 | 0.5122 | 0.5122 | 0.5059 | 0.492–0.512 | 0.489–0.516 | 0.505–0.520 |

**The two axes do not behave alike, and the gap is large.**  `num_uv_groups=4`'s three seeds cluster in a 28.85–28.95 ppl band — the tightest seed-to-seed agreement of any result in this paper — sitting entirely below UV256's single-pair range and closing roughly half the remaining gap to the MHA/GQA band.  `layers-per-m=4`, by contrast, is the *worst* OWT-ppl result of any BLT-family variant tested in this paper, worse even than every single-shared-M full-rank BLT seed it was built from.  Relaxing cross-layer sharing did not just fail to help; it actively hurt relative to keeping one M for the whole model.  A plausible mechanism: three separately-optimized M's each receive gradient updates from only a third of the network's layers per step, versus one M receiving updates from all twelve — a cost the head-axis variant never pays, since every (U,V) group, however many there are, is still shared across the full depth and gets gradient signal from every layer on every step.

**This settles the axis question this experiment was designed to answer.**  Combined with Section 4.7's indirect evidence, BLT's redundancy is concentrated in **within-layer head multiplicity**, not in the number of independently-parameterized attention computations across layers — relaxing the former recovers real quality at modest cost, relaxing the latter does not help and can actively hurt.  This is a more specific and more actionable claim than "attention has too many parameters": it says a compression scheme spending its parameter budget on per-layer specialization is spending it in the wrong place, while one spending it on per-head specialization is closer to where the redundancy actually lives.

**Not evidence for depth-position claims.**  A tempting further reading is that this result speaks to whether *later* layers specifically tolerate less capacity (as in PyramidKV-style KV-cache compression schedules).  It doesn't: `layers-per-m=4`'s strided assignment means each of its three M's serves layers spanning the *entire* depth equally, so this experiment tests only "does relaxing cross-layer sharing at all help," not "do later layers need less capacity."  The hybrid (Section 4.5) is the experiment that actually speaks to depth position, and it does support a PyramidKV-style claim: converting only the *later* half of the network's layers to full attention recovered over 75% of BLT's OWT-loss gap, well above the ~50% a depth-independent cost would predict.

**A same-hardware cost asymmetry worth noting.**  Both variants were also measured on the same P100 hardware class (`titan` for `num_uv_groups=4`, `venus` for `layers-per-m=4`): `layers-per-m=4` trains at ~722 ms/step, `num_uv_groups=4` at ~902 ms/step — the layer-axis variant is ~25% faster.  This is the expected direction: `layers-per-m=4` still computes exactly one shared bilinear score per layer (cycling through three different full-rank M's depending on depth, no extra per-step arithmetic over vanilla single-shared-M BLT), while `num_uv_groups=4` computes four separate rank-256 score matrices per layer — genuinely more FLOPs, and already eating into a real fraction of low-rank BLT's speed advantage over standard MHA (1,052 ms/step on the same hardware class).  The practical trade-off runs opposite to the quality one: the axis that helps quality costs more compute per step, and the axis that's nearly free per step doesn't help quality at all.
...
**Why BLT: memory and bandwidth, with a small and recoverable accuracy cost.** BLT's primary motivation is architectural rather than a direct accuracy claim: it cuts attention parameters by 48%, and because M is a single D×D matrix small enough to live in GPU L2 cache at GPT-2 scale (1.2 MB vs. V100's 6 MB / H100's 50 MB), it removes the need to stream per-layer W_q and W_k from HBM — a cost standard MHA pays on every layer, every decode step.  BLT also does strictly less arithmetic: one fewer D×D matmul per layer.

**A worse-than-MHA result is not the same as a useless result.** GQA is the clearest precedent for this: it underperforms standard MHA on at least one metric in every comparison in this paper (Sections 4.3–4.6), yet it is among the most widely deployed attention variants in production LLMs (LLaMA-2 70B, LLaMA-3, and others), precisely because a modest, well-characterized quality cost is judged worth its bandwidth and KV-cache savings — GQA's own positioning paper frames it explicitly as a quality/efficiency trade-off, not a claim that GQA beats MHA outright (Ainslie et al., EMNLP 2023, Section 5).  That trade-off also tends to improve with scale: GQA's quality cost is known to shrink as models grow, which is part of why it sees heavier use at the 70B+ scale than at GPT-2 scale (Section 7.1).  BLT's ~0.10 nat cost in this paper, comparable in size to GQA's own cost relative to MHA, should be read the same way — not as a demonstration that BLT should replace MHA unconditionally, but as a viable point on the same kind of quality/memory trade-off curve that already has production precedent, with the same plausible (though here untested) reason to expect the cost to shrink further at scale (Section 4.4).

**A real decode-latency measurement.** The training-time step costs reported elsewhere in this paper (444–593 ms/step, Section 3) were measured across different machines and don't give a clean training-throughput comparison.  To get an apples-to-apples number, we implemented incremental KV-cache decoding for BLT, GQA, and the hybrid — none of the training-time code exploits caching (see "KV-cache compatibility" below) — and benchmarked autoregressive decode latency for all four trained checkpoints on the same GPU (Tesla V100-DGXS-16GB), same precision, same harness:

| Config | GPT-2 (MHA) | BLT | GQA | Hybrid (6+6) |
|--------|-------------|-----|-----|--------------|
| batch 1, prompt 128 | 131.7 tok/s | **152.9 tok/s** (+16%) | 126.0 tok/s (−4%) | 137.6 tok/s (+5%) |
| batch 8, prompt 128 | 1140.0 tok/s | **1326.8 tok/s** (+16%) | 1067.2 tok/s (−6%) | 1232.5 tok/s (+8%) |
| batch 1, prompt 900 | 130.8 tok/s | **154.6 tok/s** (+18%) | 128.2 tok/s (−2%) | 143.9 tok/s (+10%) |

Figures are averaged over two back-to-back runs on an otherwise-idle GPU (we paused a concurrent training job first — an initial measurement taken while that job was still running inflated every architecture's latency by similar-or-different amounts and was discarded).  BLT decodes 16–18% faster than standard MHA across every batch size and context length tested; the hybrid gets roughly half that speedup (5–10%), consistent with only half its layers carrying BLT's benefit.  GQA, by contrast, is slightly *slower* than standard MHA here (2–6%) — its KV-group `repeat_interleave` overhead apparently outweighs its smaller K/V tensors at this scale in an unfused implementation, consistent with GQA's benefit being primarily KV-cache *size* rather than decode FLOPs.  This is a research-grade, un-fused PyTorch implementation (no custom CUDA kernels, no `torch.compile`) and isolates attention-layer cost specifically, not a production inference stack — but it is a real same-hardware measurement, not the structural argument alone.

**Adding UV256: a second same-hardware set, on different hardware than the table above.** `kv_cache_bench.py` did not originally support the low-rank attention class; we extended it with a cached decode path for UV256 (cache the V-projected key at rank r instead of the raw D-dim hidden state) and verified the cached path reproduces the model's own non-cached forward pass exactly (max abs logit diff 5.7e-06 on CPU, random-init weights) before trusting any timing off of it.  We then re-ran all five architectures — MHA, full-rank BLT, GQA, the hybrid, and UV256 — together on a Tesla P100 (`titan`), since the V100 above was mid-run on unrelated training at the time.  **These numbers are not directly comparable to the V100 table above in absolute terms** (different GPU generation, no Tensor Cores on the P100) — they are only mutually comparable *within this second set*, all five measured back-to-back on the same idle P100, same precision, same harness:

| Config | GPT-2 (MHA) | BLT | GQA | Hybrid (6+6) | UV256 |
|--------|-------------|-----|-----|--------------|-------|
| bs=1, prompt=128 — TTFT | 6.86 ms | 6.14 ms | 6.77 ms | 6.49 ms | 6.30 ms |
| bs=1, prompt=128 — decode | 320.2 tok/s | 350.1 tok/s | 345.8 tok/s | 335.5 tok/s | **356.6 tok/s** |
| bs=8, prompt=128 — TTFT | 36.32 ms | 32.37 ms | 32.48 ms | 34.35 ms | **30.85 ms** |
| bs=8, prompt=128 — decode | 1707.2 tok/s | 1898.5 tok/s | 1742.7 tok/s | 1799.6 tok/s | 1895.6 tok/s (tied w/ BLT) |
| bs=1, prompt=900 — TTFT | 46.14 ms | 35.15 ms | 42.51 ms | 40.59 ms | **32.85 ms** |
| bs=1, prompt=900 — decode | 260.8 tok/s | 281.3 tok/s | 282.6 tok/s | 271.2 tok/s | **301.4 tok/s** |

Every correctness check passed for all five architectures on this hardware too. Two predictions followed directly from the score-computation FLOPs: standard MHA and full-rank BLT both cost O(D² + D·L) per decode step (a D×D-scale projection plus one O(D) dot product against each of L cached keys); UV256 costs O(D·r + r·L) — at r=256, D=768, a 3× reduction on both terms, with the *L-dependent* term shrinking too, so UV256's edge over full-rank BLT specifically should widen with context length rather than stay flat. It does: UV256 is roughly tied with full BLT at prompt=128 (+1.9% decode, −2.6% TTFT — within noise either way) but pulls to a clear lead at prompt=900 (+7.1% decode, **+6.5% TTFT**). Prefill exposes this even more sharply than decode, since building the full L×L score matrix scales with L² rather than L — UV256's TTFT edge over standard MHA grows from 8.2% (prompt=128) to **28.8%** (prompt=900), a much larger swing than the corresponding decode-throughput gap (11.4% → 15.6%) over the same context range. Same underlying mechanism as the decode numbers above, just more exposed at the prefill stage.

None of this would be worth much if it came at a large accuracy cost, which is why most of this paper's experiments are aimed specifically at measuring that cost and showing it can be kept small: a ~0.10 nat OWT loss gap comparable in size to GQA's own cost relative to MHA (Sections 4.3–4.4), reducible to within a few percent of full MHA by the hybrid architecture (Section 4.5), and plausibly smaller still at larger scale (Section 7.1).

**Quality: BLT and GQA each give up something to MHA, but not the same thing, and not everywhere.** Section 4.3 shows BLT trails MHA by ~0.10 nats of OWT loss; GQA does not trail on this metric at all, edging slightly ahead instead.  The zero-shot benchmarks largely invert the picture: BLT is statistically tied with both GQA and MHA on HellaSwag, Winogrande, and PIQA — none of these differences clear one combined standard error (Section 4.3) — while its one real benchmark weakness is LAMBADA, where GQA's per-layer key-rank compression costs it comparable ground.  In fact the GQA-vs-MHA LAMBADA gap is the single zero-shot difference in this paper large enough to be statistically meaningful (≈2.6 combined SE).  Across the full suite, BLT and GQA each pay a real cost relative to MHA, concentrated in different places and for different reasons: BLT mainly on OWT perplexity (cross-layer M sharing, multi-seed confirmed), GQA mainly on LAMBADA (compressed per-layer key rank, statistically real).  Neither is decisively better than the other, and neither uniformly trails MHA either; the right choice depends on whether parameter count or KV-cache size is the binding constraint.

**The hybrid is the best quality/memory trade-off tested, not a free lunch.** Section 4.5's 6 MHA + 6 BLT hybrid posts a statistically real (≈2.3 SE) LAMBADA edge over GQA, ties it within noise on HellaSwag, Winogrande, and OWT perplexity, and is within noise of full MHA on every zero-shot task, while still saving ~23% of attention parameters.  The gap recovery is disproportionate to the layer count converted — half the BLT layers recover over 75% of the OWT loss gap — which rules out a flat additive cost, though with only one intermediate split tested we can't yet pin down the exact functional form (Section 7.2): six unconstrained layers are apparently enough for the model to mostly route around M's limitations even with M still present elsewhere.  But the hybrid keeps only half of BLT's memory benefit; it is a point on a curve between BLT and MHA, and where to sit on that curve is a deployment choice this paper does not resolve.
...
**`num_uv_groups`'s TP bandwidth bet is real, but only at scales larger than we can afford to train.** A separate, medium-scale (355M-parameter) `num_uv_groups=8` model was trained specifically for deployability: G=8 groups, each independently shardable to one GPU under 8-way TP with no replication, unlike full-rank M (above); it matched standard MHA's LAMBADA accuracy closely at a small OWT-perplexity cost, but trained 17-20% slower than MHA at this width (not otherwise reported in this paper, which focuses on the small-scale from-scratch comparison). Sharding alone does not decide whether this design actually beats standard MHA's own bandwidth at matched TP degree, though — both a G-group design (1 group/GPU) and head-sharded MHA cache `(G·r+D)/N` and `2D/N` bytes per token per GPU respectively, and since both terms scale identically as `1/N`, their *ratio*, `(G·r+D)/(2D)`, is independent of TP degree. It depends only on how the fixed per-group rank `r` compares to the model width `D`. At the medium scale just described (`D=1024`, `r=256`, `G=8`), that ratio is 1.5 — `num_uv_groups=8` loses to MHA's per-GPU bandwidth at *any* TP degree, consistent with the observed training slowdown. At a 7B-class width (`D=8192`, same `r=256`), the identical formula gives 0.625 — a 37.5% saving — because the fixed-rank overhead per group becomes a much smaller fraction of a much larger `D`.

We cannot train a 7B model to confirm this directly, but the underlying claim is architectural (wall-clock and bytes moved), not a quality claim, so it doesn't require trained weights to test — the same principle behind every decode-latency measurement in this section. We isolated exactly the computation one GPU would run under 8-way `num_uv_groups` TP (U, V ∈ D×r, W_v ∈ D×(D/8)) against the equivalent MHA shard (W_q, W_k, W_v ∈ D×(D/8)), and measured real single-GPU decode throughput, sweeping `D` from 768 to 8192 with `r` fixed at 256 (batch=1, 200 decode steps, two context lengths):
...
The empirical crossover lands almost exactly at `D = G·r = 2048`, matching the formula's prediction: below it the UV shard is at parity or slower (worse at long context specifically, where per-step cache-read cost dominates over one-time weight load), above it the advantage grows quickly, reaching a real ~49-53% faster decode at `D=8192` regardless of context length. This confirms on real hardware — not just in the byte-counting model — that `num_uv_groups=8`'s bandwidth motivation is specifically a large-model-scale bet: sound at 7B-class widths, not yet realized at the model sizes this project can actually afford to train from scratch. (This is a distinct design point from the globally-shared `G=1` UV^T variant of Section 7.3, whose own TP story doesn't depend on this same rank-vs-width crossover, since its single rank-r pair is replicated rather than sharded.)

**KV-cache compatibility.** Standard KV caching stores K = X W_k for past tokens.  In BLT, the score between new token t and past token j is (x_t M) · x_j, so the cache only needs to store the raw hidden states x_j (as implicit keys) and x_j W_v (as values).  A single M multiply produces the query; no key projection is required when adding new tokens.  The training-time code (Sections 1–4) does not exploit this and recomputes full attention from scratch at each forward pass, since training processes whole sequences in parallel and has no use for incremental decoding.  We implemented this caching separately for the decode-latency benchmark above (`kv_cache_bench.py`), confirmed it reproduces the training-time code's outputs exactly (matching non-cached forward-pass logits to within floating-point tolerance for all four architectures), and used it to produce the measurements in this section.

**What this comparison shows, independent of whether BLT itself gets adopted.** A negative or mixed result is still informative, and several findings in this paper say something about transformer attention more generally, regardless of BLT's own fate:

- *Cross-layer sharing of the query-key interaction is cheap.* A single matrix M, with zero per-layer adaptation, reproduces standard MHA's OWT perplexity to within ~0.10 nats (Section 4.3) — in the same range as GQA's well-tolerated within-layer key-rank compression.  Twelve independently-learned per-layer interaction matrices turn out to be a much larger function class than the model actually needs.
- *That cost concentrates rather than spreading evenly across depth.* Converting only 6 of 12 layers back to full attention recovers over 75% of the gap (Section 4.5) — cross-layer sharing is far more tolerable in some layers than others.  This complements, but is distinct from, the whole-layer-pruning literature cited there (Gromov et al.): here, even a layer that keeps its FFN, value, and output projections fully intact can have just its *attention pattern* constrained at little cost, provided it's late enough in the network.
- *Per-head attention-pattern diversity may be less load-bearing than per-head value diversity.* BLT collapses all heads in a layer to an identical attention pattern (Section 2.3) yet remains competitive with GQA and within a few points of full MHA on every zero-shot benchmark tested (Section 4.3) — differentiation from W_v alone preserves most of what standard multi-head attention provides on these tasks.
- *The shared matrix does not collapse to a trivial solution.* Despite serving 144 simultaneous layer/head contexts, the SVD of trained M is nearly flat, not low-rank (above) — optimization finds a genuinely rich compromise rather than an averaged-out one.
- *Cross-layer sharing and within-layer rank compression are separable failure modes that cost different downstream skills.* BLT's cost concentrates on general next-token prediction; GQA's concentrates on long-range retrieval (Section 4.4).  That's a map of which compression axis costs which capability — useful input for any new compression scheme, including BLT's own proposed UV^T and multi-M variants (Section 7), which combine both axes deliberately.
- *Smaller cached tensors do not automatically mean faster decode.* GQA was slightly slower than full MHA in our decode-latency benchmark despite caching less data per token (above) — KV-group `repeat_interleave` overhead dominated at this batch/context scale in an unfused implementation, a reminder that algorithmic savings and measured wall-clock savings are not the same claim.
- *Within-layer head collapse is cheaper than cross-layer sharing — confirmed directly, not just by inference.* Section 4.7's UV256 result and Section 4.5's hybrid result each relax one of BLT's two constraints in isolation but via different mechanisms, and already pointed the same direction. Section 4.8 tests both axes with matched added capacity on the same footing and removes the ambiguity: taking `num_uv_groups` from 1 to 4 (relaxing only the head axis) closes roughly half of low-rank BLT's remaining gap to standard MHA across three seeds, while the equivalent-capacity move on the layer axis alone (three strided full-rank M's instead of one) produces the single worst OWT-ppl result of any BLT variant tested in this paper — worse than the one-M baseline it was built from. Read together with Sections 4.5 and 4.7, the redundancy this paper set out to find is concentrated in *within-layer head multiplicity*, which compresses for measurable quality gain, not merely low cost, while *cross-layer specialization* is expensive to give up and actively harmful to relax carelessly — a more specific claim than "attention has redundant parameters," and one that says which axis of the standard scheme's parameter budget a compression scheme should target first.
...
BLT (a single shared M) is one extreme of a design space; standard MHA is the other, with a distinct M_h^l for every layer and head (N×H total, Section 1).  This space has two *independent* axes — how many distinct M's to use across **heads** versus across **layers** — not one combined dial.  A model can mix any number of head-groups (1 to H) with any number of layer-blocks (1 to N); each combination is a different architecture.  Full BLT sits at (1 head-group, 1 layer-block); full MHA sits at (H head-groups, N layer-blocks).  Section 4.8 now reports one converged point on each axis (4 head-groups via low-rank UV^T; 3 layer-blocks via strided full-rank M) — the sketch below is otherwise unchanged from before that result and the remaining intermediate points are still untested.

Two corners of this space besides BLT and MHA are useful reference points:

- **One M per head, shared across all layers** (H head-groups, 1 layer-block): the endpoint of a head-grouped sweep (2, 4, 6, ... head-groups), the direct analogue of GQA's KV-group count.  BLT-2M, tested briefly in early WikiText-103 experiments with 2 head-groups, sits partway along this axis but was not carried to convergence and is not reported as a result here.  Section 4.8's `num_uv_groups=4` is a converged point further along the same axis, though via a different mechanism than this corner describes — 4 low-rank (U,V) pairs at r=256, not 4 (or ultimately H=12) full-rank M_h's — and it works: three seeds recover roughly half of low-rank BLT's remaining gap to MHA.  This endpoint (the full-rank, one-M-per-head version) also restores tensor-parallel shardability (Section 6): since each head now has its own dedicated M_h, a GPU can own a disjoint subset of heads — M_h, W_v, W_o together — with no need to replicate a shared matrix or all-reduce an attention matrix across GPUs, unlike full-rank single-M BLT.  This is a structural fix to the same TP problem that motivates the low-rank UV^T variant (Section 7.3), via a different mechanism: UV^T makes the globally-shared object small enough to replicate cheaply, while one-M-per-head avoids needing to share across the partition axis at all.  It does not, however, shrink the matrix itself — each M_h is still a full D×D matrix rather than a thin D×d projection — so the parameter savings relative to standard MHA are real but more modest than full BLT's (H×D² vs. D² for the shared QK-interaction structure).
- **N distinct M's, one per layer, shared across all heads** (1 head-group, N layer-blocks): unlike current BLT, which reuses the *same* single M at every layer, this scheme gives each of the N layers its own separate matrix (M_1, ..., M_N) — but within any one layer, every head still shares that layer's matrix, so BLT's "identical attention weights per head" constraint is unchanged.  Only the cross-layer sharing constraint is relaxed, and only partially: from 1 matrix used N times down to N matrices used once each.  Section 4.8's `layers-per-m=4` is a converged point at N=3 (strided across the 12 layers rather than a contiguous block), and it does not work: OWT ppl is the worst of any BLT variant tested in this paper, worse than the N=1 baseline it was built from.  The hybrid in Section 4.5 is a related but distinct construction: rather than giving every layer its own matrix, it gives early layers no sharing at all (full per-head MHA) and late layers one shared M (full BLT) — a depth-dependent mix of *both* axes at once, not a point on this single-axis line.  Section 4.5's rationale for that depth-dependence — that later layers tolerate constraints better than early ones ([arXiv 2403.17887](https://arxiv.org/abs/2403.17887), Gromov et al., ICLR 2025) — applies to the layer axis specifically, and has no analogue for the head axis, since there is no comparable evidence that some heads matter systematically more than others.  Section 4.8's strided (not contiguous) assignment means its N=3 result cannot distinguish "the layer axis is expensive everywhere" from "the layer axis is expensive only early" — only the hybrid's depth-ordered construction speaks to that.

**These two axes have very different memory-bandwidth consequences, and the difference is about layers, not heads.** What determines BLT's weight-streaming cost is how many *distinct* M's must be loaded from HBM over one forward pass — i.e., the number of layer-blocks, independent of how many head-groups are used.  All of a layer's head-groups are needed at the same point in the forward pass, so however many there are, they load together as one (larger) weight fetch — still just once, reused across all N layers exactly as with a single M, only G_head times the bytes.  Increasing the number of layer-blocks does the opposite: it directly increases the number of separate M-loads per forward pass, one per block, each amortized only across the layers in its own block.  So moving along the head axis is nearly free on the memory side; moving along the layer axis is not, regardless of how many head-groups (if any) are combined with it.  The hybrid's 6 MHA layers already pay this cost in Section 4.5; giving every layer its own distinct M would pay it at every layer simultaneously, trading away essentially all of BLT's cross-layer cache-reuse benefit in exchange for per-layer (but still per-head-shared) specialization.  Section 4.8's compute-side finding (the head axis costs more FLOPs per step, not more bandwidth) is a separate, orthogonal cost from this bandwidth argument — the two together mean the head axis is cheap on bandwidth but not on compute, while the layer axis is expensive on bandwidth and does not even buy quality in return.

Section 4.8 answers the direction of this trade-off — head-groups help, layer-blocks don't — but not its shape.  A fuller sweep along the head axis specifically (2, 4, 6, ... up to H=12 head-groups, or full-rank M_h's rather than low-rank pairs) — trained from scratch to full convergence on OWT under the same protocol as Section 4.3 — would map out where returns diminish and where the extra per-step compute (Section 4.8) stops being worth the quality gain.  Whether the resulting curve dominates GQA's quality/cache-size curve at a matched budget is an open question this paper does not answer.
...
This isn't free, though.  The SVD analysis in Section 6 shows M's singular spectrum is nearly flat (550 of 768 singular values exceed 10% of the maximum), so a UV^T model cannot simply be initialized by truncating a trained full-rank M's spectrum — r=256 would capture only 81% of Frobenius energy at GPT-2 scale.  Truncating and fine-tuning (factorize a trained M ≈ UV^T at r=192 or r=256, then fine-tune for ~50–100K additional steps) remains a plausible cheaper path we have not tried; instead, our first result trains UV^T from scratch at r=256, avoiding both the truncation quality hit and this project's repeated bad experiences with warm-started initialization.  Three from-scratch seeds now confirm UV256 beats full-rank BLT with no overlap between the seed ranges (Section 4.7), matches standard MHA within noise on three of four zero-shot benchmarks despite roughly half the attention parameters, and trains 32% faster than MHA on identical hardware — the same three-seed standard as the rest of the paper.  Extending the same low-rank machinery to multiple (U,V) pairs (Section 4.8) further shows the within-layer head axis, not the cross-layer axis, is where relaxing BLT's constraints actually pays off.
