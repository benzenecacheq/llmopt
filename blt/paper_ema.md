...
**This was the best single-seed result in the EMA/blend family at the time it was run.** The jointly-trained 75/25 blend beats even *pure* EMA's LAMBADA accuracy (0.269 vs. 0.253) — a result no sequential fine-tune at any α reached — while landing at essentially the same OWT-ppl cost as the best sequential fine-tune tested (29.61 vs. 29.68 for the 50/50 sequential blend). This confirms the hypothesis motivating the experiment: training the blended objective from scratch avoids the forgetting-cliff dynamics of Section 4.2 entirely (there is no pure-EMA basin to escape from), and reaches a Pareto-better point than any fine-tune-after-convergence protocol.

**Methodological note: verify EMA is actually active before trusting a run.** A first attempted replicate at seed 7 (`run_gpt2_ema_blend75_scratch_seed7.pt`, launched on `titan`) finished 2026-07-06 with numbers that superficially looked like a "mirror image" of the seed42 result — OWT ppl 28.18, LAMBADA acc 0.203, both closer to the non-EMA baseline than to seed42's blend result. Direct inspection of the checkpoint's `ema_loss` buffer explained why: it sat bit-for-bit at its untouched initialization value (`log(vocab_size)`, std exactly 0.0) across all 50,257 vocabulary entries — proof `--ema-loss-weighting` was never active for a single step. The launch had passed `--ema-blend 0.75` without `--ema-loss-weighting`, which is a complete no-op (`--ema-blend` only has an effect inside the `if args.ema_loss_weighting:` branch); the run trained as an ordinary baseline for all 500K steps, on different hardware than the true seed-7 baseline, fully explaining the baseline-like numbers. **This run is retracted and excluded from the results below.** Lesson applied to every subsequent EMA run in this paper: verify the checkpoint's `ema_loss` buffer shows genuine per-token spread (not std=0.0) at the first checkpoint save, rather than discovering a launch-flag mistake only after a full run and benchmark.

Two genuine replicates were then run, each verified correct early via the checkpoint-buffer check above: seed 19 (`run_gpt2_ema_blend75_scratch_seed19.pt`, DONE 2026-07-13) and seed 7_v2 (`run_gpt2_ema_blend75_scratch_seed7_v2.pt`, run on `venus`, DONE 2026-07-18).

| | seed42 | seed19 | seed7_v2 | Non-EMA baseline |
|---|---|---|---|---|
| OWT held-out ppl | 29.61 | 29.61 | 29.42 | 27.78 |
| OWT held-out loss | 3.3881 | 3.3882 | 3.3818 | 3.3243 |
| LAMBADA acc | 0.269 | 0.253 | 0.257 | 0.225 |
| LAMBADA ppl | 130.6 | 127.8 | 131.3 | 174.6 |
| HellaSwag acc_norm | 0.273 | 0.271 | 0.266 | 0.268 |
| PIQA acc_norm | 0.565 | 0.582 | 0.574 | 0.579 |
| Winogrande acc | 0.522 | 0.508 | 0.520 | 0.505 |

**All three genuine seeds land solidly above the non-EMA baseline's LAMBADA accuracy (0.225)** — 0.269, 0.253, 0.257, averaging 0.260 — a real three-seed confirmation of the jointly-trained 75/25-blend finding, not a single uncontested data point. OWT ppl is tight and consistent across all three (29.42–29.61). This is now a solid basis for this paper's central claim, not preliminary evidence.

### 4.5 Annealing the Blend Coefficient Improves LAMBADA Further

Section 4.4 fixes the blend coefficient at α=0.75 for the entire training run. But the EMA buffer itself is least trustworthy early in training — each vocabulary token has seen only a handful of updates, so its per-token EMA value is a noisy estimate rather than a reliable "this token is hard" signal. This motivates **annealing** α from 0 up to its target over the course of training, rather than applying full-strength reweighting from step 0, so the model spends early training on uncorrupted uniform-CE signal and only leans on the reweighting once the EMA buffer has had time to differentiate.

**Mechanism** (`train.py --ema-blend-schedule sine`; default `fixed` exactly preserves Section 4.4's behavior). The effective blend at step `s` is `ema_blend · sin(0.5π · min(s, max_steps)/max_steps)` — 0 at step 0, ramping smoothly to exactly the target `ema_blend` value at `max_steps`. The actual per-step effective blend is logged directly (`effective_blend` training-log column) rather than only inferred from the formula, and was confirmed to track the formula exactly in a live run (e.g. `effective_blend=0.0753` at step 32,000 of a 500,000-step, target-0.75 run, matching `0.75 · sin(0.5π · 32000/500000)` computed by hand).

We trained GPT-2 from scratch with `--ema-loss-weighting --ema-blend 0.75 --ema-blend-schedule sine`, seed 42, otherwise identical to Section 4.4's protocol: `run_gpt2_ema_blend75_sine_scratch_seed42.pt`, DONE 2026-07-27 (`effective_blend` hit exactly 0.7500 at the final step, confirming the schedule ran correctly start to finish).
...
**The sine schedule beats every fixed-blend seed on LAMBADA accuracy (0.281 vs. 0.253–0.269) at essentially the same OWT-ppl cost.** 29.44 sits comfortably inside the fixed-blend seeds' own 29.42–29.61 range — not a worse trade-off point purchased for the LAMBADA gain, but a genuinely better point on the same curve. This matches the motivating hypothesis: ramping the EMA weight in gradually, rather than applying it at full target strength while the per-token buffer is still uninformative, lets the model spend early training on clean signal and only lean on the reweighting once it is trustworthy.
...
**Annealing is a variant of the same mechanism, not a new one.** The sine schedule (Section 4.5) does not introduce a new idea beyond blending — it is the same fixed-target blend as Section 4.4, just reached gradually rather than immediately. That it outperforms a fixed schedule to the same target is consistent with the basin-formation story above: even a blended (not pure) objective, applied at full strength before the EMA buffer has differentiated, still shapes early training around an under-informed weighting signal. Ramping the weight in avoids that specific failure mode without touching anything else about the method.

**Cost is bounded, not runaway.** Across every configuration tested — pure EMA, every blend ratio, both architectures — OWT held-out perplexity degradation stays in a narrow band (roughly +5% to +11% relative to the matched non-EMA baseline). There is no configuration where the EMA mechanism causes catastrophic degradation of general language-modeling ability; the cost is a real but modest, controllable tax, and the 75/25 jointly-trained blend — and the sine-annealed version of it — suggest that tax can be made even smaller while *keeping* essentially all of the LAMBADA benefit.

**Open questions this paper does not resolve** (carried forward from `project_loss_function_ideas` memory and CLAUDE.md's "Active run" notes, updated for what Sections 4.4-4.5 have since answered):

- *(Answered)* Does a jointly-trained blend beat sequential fine-tune? — **Yes** (Section 4.4), now confirmed across three independent seeds (LAMBADA acc 0.269/0.253/0.257, mean 0.260, all above the 0.225 baseline).
- *(Answered, single seed)* Does annealing the blend coefficient in via a schedule help further? — **Yes, in a first run** (Section 4.5): a sine anneal to the same α=0.75 target reaches the best LAMBADA accuracy in the entire EMA/blend family (0.281) at no extra OWT-ppl cost. **Not yet seed-confirmed** — a second sine seed is in progress on `venus`.
- *(Partially answered)* Sweep more α values — Section 4.3 covers 0.25/0.5/0.75 sequential; Section 4.4 covers only α=0.75 jointly-trained; Section 4.5 covers only a sine anneal to a 0.75 target. A full sweep of jointly-trained fixed α values, and of sine-anneal targets other than 0.75, has not been run.
- *(Open)* Is either the jointly-trained-blend result (4.4) or the sine-anneal result (4.5) architecture-general the way the base EMA effect was (Section 4.1)? No BLT-blend experiment — sequential, jointly-trained, or sine-annealed — has been run yet; only pure EMA (α=1.0) has been tested on BLT.
...
| α=0.75 jointly-trained-from-scratch (seed 19) | `run_gpt2_ema_blend75_scratch_seed19.pt` (on `titan`) | on `titan` | `lm_eval_gpt2_ema_blend75_scratch_seed19.json` | — |
| α=0.75 jointly-trained-from-scratch (seed 7_v2) | `run_gpt2_ema_blend75_scratch_seed7_v2.pt` (on `venus`) | on `venus` | `lm_eval_gpt2_ema_blend75_scratch_seed7_v2.json` | OWT eval relayed to `io` (no local parquet on `venus`) — `eval_owt_gpt2_ema_blend75_scratch_seed7.stdout` |
| **RETRACTED** — seed-7 launch missing `--ema-loss-weighting` (see Section 4.4 methodological note; excluded from all results) | `run_gpt2_ema_blend75_scratch_seed7.pt` (on `titan`) | `run_gpt2_ema_blend75_scratch_seed7.log` (on `titan`) | numbers not used — checkpoint `ema_loss` buffer confirms EMA was never active (std=0.0) | — |
| α=0.75 sine-annealed, jointly-trained-from-scratch (seed 42) | `run_gpt2_ema_blend75_sine_scratch_seed42.pt` (on `venus`) | on `venus` | `lm_eval_gpt2_ema_blend75_sine_scratch_seed42.json` | `eval_owt_gpt2_ema_blend75_sine_scratch_seed42.stdout` |
| α=0.75 sine-annealed, jointly-trained-from-scratch (seed 19, IN PROGRESS) | `run_gpt2_ema_blend75_sine_scratch_seed19.pt` (on `venus`) | on `venus` | not yet run | not yet run |
...
**Code references**: EMA mechanism in `train.py` (search `ema_loss`, `ema_blend`, `ema_decay`; flags defined ~line 420-425, forward-pass logic ~line 289-312). `--finetune`'s EMA-buffer-loading fix (previously only `--resume` restored it) landed alongside `--ema-blend` in commit `c118787`. `--ema-blend-schedule sine` (Section 4.5) and the `effective_blend` training-log column landed 2026-07-21.
