...
- **Ground-truth evaluation as the wrong criterion**: Standard benchmarks measure whether a compressed model gives the correct answer, not whether it behaves the same way as the full model. We argue this is the wrong objective for compression and analyze four distinct failure modes of GT evaluation as a compression metric (§5.1).
...
Table 6 gives the same short/long breakdown by KL, showing what F_out cannot.
...
On short outputs, unrotated SnapKV and PyramidKV produce KL of 1.3–1.7 — far worse than Naive (0.143) — despite leading on F_out. The first-token advantage explains the inversion: F_out is dominated by the first output word, which is identical to the full model's by construction, masking severe distributional corruption at all subsequent positions. Re-rotation reduces short-output KL to 0.012 — near-perfect fidelity. On long outputs, where F_out converges (re-rotated ~50% vs unrotated ~55%), KL still shows a 6× improvement from re-rotation (0.124 vs 0.697). The apparent trade-off visible in F_out — re-rotated methods losing a few points at both lengths — is precisely the metric inversion this paper documents: F_out is dominated by short tasks where the first-token advantage gives unrotated methods a structural edge; in both output regimes, KL correctly identifies re-rotation as the more faithful compression.
