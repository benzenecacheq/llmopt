...
BLT (a single shared M) is one extreme of a spectrum; standard MHA is the other, with a distinct M_h^l for every layer and head (N×H total, Section 1).  There are two natural ways to sit partway between the two: group by **head** or group by **layer**.  Neither has been trained to convergence in this paper; both are flagged purely as directions worth exploring.

**Head-grouped.** Split the H heads into a small number of groups, each with its own M shared across all N layers — the scheme tested briefly in early WikiText-103 experiments as BLT-2M (two groups), but not carried to convergence and not reported as a result here.  A fuller sweep (2, 4, 6 head-groups), trained from scratch to full convergence on OWT under the same protocol as Section 4.3, would map out the quality/parameter curve as the number of shared M's grows from 1 (full BLT) toward H (standard MHA per head) — the direct analogue of GQA's KV-group count.

**Layer-grouped.** The alternative is to split the N layers into contiguous blocks instead, each block sharing one M across all its heads.  The hybrid experiment (Section 4.5) is the most extreme 2-block case of this: the early block uses no sharing at all (standard MHA) and the late block uses full sharing (full BLT).  This grouping is motivated by evidence that, unlike heads, layers are *not* interchangeable in importance — Section 4.5 cites Gromov et al.'s finding that later transformer layers tolerate pruning far better than early ones ([arXiv 2403.17887](https://arxiv.org/abs/2403.17887), ICLR 2025).  If later layers are systematically less critical, concentrating M's shared capacity there and relaxing it in early layers should strike a better quality/parameter balance than head-grouping, which has no comparable evidence of asymmetric importance to exploit.  The natural next experiment is the interior of this design space: more than 2 layer-blocks, with finer-grained, depth-dependent sharing rather than the hybrid's binary split.

**The memory-bandwidth trade-off is not symmetric between the two.** Head-grouping preserves BLT's cache-residency advantage almost entirely: every M is still reused across all N layers, so the cache footprint grows only linearly in the number of groups (G×D² total, still amortized N times each) — the same caching argument as full BLT, just G times the matrix data resident instead of 1×.  Layer-grouping does not get this for free: each M is now reused only across the layers in its own block, so whichever layers are given more capacity (smaller blocks, or no sharing at all) are exactly the layers that lose the cross-layer reuse benefit — the same cost the hybrid already pays in Section 4.5, where the 6 MHA layers behave exactly like standard MHA on the memory side.  A graduated, depth-dependent layer-grouping scheme inherits this trade-off at every early layer it makes more expressive: there is no way to give early layers more capacity without also giving up BLT's bandwidth advantage specifically where the model needs the capacity most.  Layer-grouping is the better-motivated scheme for recovering quality per parameter spent; head-grouping is the better-motivated scheme for preserving memory bandwidth.  Which trade-off is preferable depends on whether the deployment is parameter-bound or bandwidth-bound — and, as with the hybrid, whether multi-M BLT's resulting quality/parameter (or quality/bandwidth) curve dominates GQA's quality/cache-size curve at a matched budget is an open question this paper does not answer.
