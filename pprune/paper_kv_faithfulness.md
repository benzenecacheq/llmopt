...
To confirm that mechanism rather than token selection drives the faithfulness gap, we compare SnapKV KV pruning to SnapKV-Select — identical attention-score-based token selection, presented as a new prompt rather than patched into the KV cache (Table 1). The same tokens, selected by the same algorithm, behave very differently depending on how they are presented to the model. Table 1 also includes a third SnapKV variant (pcfg) that restricts the attention mask during prefill so the model only ever attends to the selected positions — no post-prefill eviction, no enrichment from evicted tokens, no KV patching. This isolates whether the enrichment effect (§6.1) rather than the eviction mechanism is responsible for the damage.
