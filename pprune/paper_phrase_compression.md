...
This asymmetry has a structural explanation (developed in §9.4): PyramidKV performs a full prefill over the uncompressed prompt before pruning the KV cache in-place, so its output distribution at the first generated token is identical to the full model's by construction. On short-answer tasks where the answer is a single token or short phrase — a class label, a retrieved passage, a trivia answer — the first token IS the answer, so F_out is essentially 1.0 whenever the full model is also correct. The average F_out of 79.4% is substantially driven by these tasks dominating the 16-task mix.
