...
- **One M per head, shared across all layers** (H head-groups, 1 layer-block): the endpoint of a head-grouped sweep (2, 4, 6, ... head-groups), the direct analogue of GQA's KV-group count.  BLT-2M, tested briefly in early WikiText-103 experiments with 2 head-groups, sits partway along this axis but was not carried to convergence and is not reported as a result here.
