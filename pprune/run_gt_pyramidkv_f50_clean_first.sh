#!/bin/bash
# GT evaluation for pyramidkv_f50_clean_first: PyramidKV at 50% retention
# with the clean-first-token fix (T1 attends to compressed KV, not full prefill).
#
# Output → lb_results_base/gt_pyramidkv_f50_clean_first/

set -e
LOG=/home/benzene/llmopt/pprune/lb_results_base/gt_pyramidkv_f50_clean_first.log
exec >> "$LOG" 2>&1

echo "=== run_gt_pyramidkv_f50_clean_first.sh started at $(date) ==="
...
python gt_eval_compression.py \
    ...
    --methods pyramidkv_f50_clean_first \
    --tasks "$TASKS" \
    --output lb_results_base/gt_pyramidkv_f50_clean_first \
    --n 100

echo "=== run_gt_pyramidkv_f50_clean_first.sh complete at $(date) ==="
